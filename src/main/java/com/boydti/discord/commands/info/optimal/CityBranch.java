package com.boydti.discord.commands.info.optimal;
...
import static com.boydti.discord.apiv1.enums.city.building.Buildings.COAL_POWER;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.HOSPITAL;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.NUCLEAR_POWER;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.OIL_POWER;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.OIL_WELL;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.POLICE_STATION;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.RECYCLING_CENTER;
import static com.boydti.discord.apiv1.enums.city.building.Buildings.WIND_POWER;
    ...
    public void accept(Map.Entry<JavaCity, Integer> originPair, PriorityQueue<Map.Entry<JavaCity, Integer>> cities) {
        ...
        if (originPair.getValue() <= 4) {
            ...
            if (unpowered > 500 || (unpowered > 250 && origin.getInfra() > 2000)) {
                cities.enqueue(create(originPair, NUCLEAR_POWER, 4));
                return;
            }
            if (unpowered > 250) {
                if (OIL_WELL.canBuild(continent)) {
                    cities.enqueue(create(originPair, OIL_POWER, 4));
                } else {
                    cities.enqueue(create(originPair, COAL_POWER, 4));
                }
                return;
            }
            if (unpowered > 0) {
                cities.enqueue(create(originPair, WIND_POWER, 4));
                return;
            }
        }
...
//                    }
                ...
                } else if (building == RECYCLING_CENTER) {
                    Integer pollution = origin.getPollution(hasProject);
                    if (pollution > 0) {
                        cities.enqueue(create(originPair, building, i));
                    }
                } else if (building == POLICE_STATION) {
