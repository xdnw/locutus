...
public class ROI extends Command {
    ...
    @Override
    public String help() {
        return "!" + getClass().getSimpleName() + " <alliance|nation|tax-url|*> [days]";
    }
    ...
    public String onCommand(MessageReceivedEvent event, Guild guild, User author, DBNation me, List<String> args, Set<Character> flags) throws Exception {
        ...
        if (guildDb == null || guildDb.getInfo(GuildDB.Key.ALLIANCE_ID) == null) {
            return "Invalid guild. Please register your alliance id with: `!keystore ALLIANCE_ID <value>`";
        }

        Message message = com.boydti.discord.util.RateLimitUtil.complete(event.getChannel().sendMessage("Fetching nations: "));

        String allianceStr = guildDb.getInfo(GuildDB.Key.ALLIANCE_ID);
        if (allianceStr == null) return "Please use `!keystore ALLIANCE_ID <alliance-id>`";
            ...
            if (optimal != null) {
                ...
                if (baseOptimizedProfit > baseProfit) {
                    ...
                    if (netProfit > 0 && !optimal.equals(existingCity)) {
                        ...
                        roiMap.add(new ROIResult(nation, Investment.BUILD, "!OptimalBuild " + days + " <city-url>", roi, cost, netProfit, profit));
