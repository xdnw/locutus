...
public class WikiCustomEmbeds extends BotWikiGen {
    ...
    public String generateMarkdown() {
        return build(
                ...
                "Example: " + CM.embed.add.command.cmd.create("<url>", "Blah", CommandBehavior.EPHEMERAL.name(), CM.who.cmd.getPath(), "nationoralliances").toString(),
                "# Add a modal (prompt button) to an embed",
                commandMarkdownSpoiler(CM.embed.add.modal.cmd),
                "Example with default:",
                CM.embed.add.modal.cmd.create("<url>", "Who", CommandBehavior.EPHEMERAL.name(), CM.who.cmd.getPath(), "nationoralliances", "list: True").toString(),
                "Example with placeholder, and default",
                CM.embed.add.modal.cmd.create("<url>", "Who Cities", CommandBehavior.EPHEMERAL.name(), CM.who.cmd.getPath(), "num_cities,position", "nationoralliances: Rose,#cities>{num_cities},#position={position=1}").toString(),
