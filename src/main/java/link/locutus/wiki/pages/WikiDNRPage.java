    ...
    public String generateMarkdown() {
        return build(
            "Mark an alliance's members or applicants as Do Not Raid (DNR). " +
                ...
                Coalition.IGNORE_FA.getDesc(),
                CM.coalition.create.cmd.alliances("AA:Camelot").coalitionName(Coalition.IGNORE_FA.name()).toString(),
                "# Ping milcom for an alliances violations",
                Coalition.COUNTER.getDesc(),
                CM.coalition.create.cmd.alliances("AA:Guardian").coalitionName(Coalition.COUNTER.name()).toString(),
                "# Instruct FA to attempt peace",
                Coalition.FA_FIRST.getDesc(),
                CM.coalition.create.cmd.alliances("AA:Arrgh").coalitionName(Coalition.FA_FIRST.name()).toString(),
                ...
                CM.coalition.remove.cmd.alliances("AA:Eclipse").coalitionName(Coalition.DNR.name()).toString(),
                "If they are not added, but are DNR by `" + GuildKey.DO_NOT_RAID_TOP_X.name() + "`:",
                CM.coalition.create.cmd.alliances("AA:Rose").coalitionName(Coalition.CAN_RAID.name()).toString(),
                "Exclude inactives (1w):",
                CM.coalition.create.cmd.alliances("AA:Rose").coalitionName(Coalition.CAN_RAID_INACTIVE.name()).toString(),
                ...
                CM.coalition.create.cmd.alliances("AA:The Knights Radiant").coalitionName(Coalition.DNR_MEMBER.name()).toString(),
                "# DNR only inactive",
                "Add a time difference to the coalition name, like so: `DNR_1w` or `DNR_MEMBER_30d`",
                CM.coalition.create.cmd.alliances("AA:Aurora").coalitionName("DNR_1w").toString(),
                "# See also",
