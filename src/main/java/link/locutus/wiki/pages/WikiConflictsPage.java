...
public class WikiConflictsPage extends BotWikiGen {
    ...
    public String generateMarkdown() {
        return build(
                ...
                "#### Auto-detect the true start turn",
                "Iterates forward from the current start (optionally a few turns earlier via `turn_allowance`) and surfaces nearby qualifying turns where at least three distinct nations on one coalition declare on the opposing coalition during that turn. The preview includes declaration totals plus per-alliance sums, and confirmation applies the earliest surfaced candidate via an opaque token without re-detecting. This does not load conflict war stats or push to the site.",
                commandMarkdownSpoiler(CM.conflict.edit.detect_start.cmd),
