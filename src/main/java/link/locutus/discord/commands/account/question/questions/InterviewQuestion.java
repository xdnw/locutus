        ...
        public boolean validate(Guild guild, User author, DBNation me, DBNation sudoer, IMessageIO channel, String input) throws IOException {
            if (!input.equalsIgnoreCase("Y")) {
                if (GuildKey.MEMBER_CAN_SET_BRACKET.getOrNull(Locutus.imp().getGuildDB(guild)) == Boolean.TRUE) {
                    ...
                    channel.sendMessage(msg, RateLimitedSources.COMMAND_RESULT);
                }
            }
            return true;
        }
                    ...
                    if (sudoer.getPosition() > 2 && sudoUser != null) {
                        if (Roles.INTERVIEWER.has(sudoUser, guild)) {
                            try {
                                ...
                                if (result != null) {
                                    channel.sendMessage(result, RateLimitedSources.COMMAND_RESULT);
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                            ...
                            if (member != null && memberRole != null) {
                                RateLimitUtil.queue(guild.addRoleToMember(member, memberRole), RateLimitedSources.COMMAND_RESULT);
                            }
                        }
                    } else {
...
//    },
    ...
    DEPOSIT_RESOURCES("""
        ...
        public boolean validate(Guild guild, User author, DBNation me, DBNation sudoer, IMessageIO channel, String input) throws IOException {
            ...
            Set<Integer> aaIds = db.getAllianceIds();
            if (!aaIds.isEmpty()) {
                List<Transaction2> transactions = me.getTransactions(0, false, 0, Long.MAX_VALUE);
