...
public class CityCost extends Command {
    ...
    @Override
    public String help() {
        return super.help() + " <current-city> <max-city> [government-support-agency=false] [domestic-affairs=false]";
    }
    ...
    public String onCommand(Guild guild, IMessageIO channel, User author, DBNation me, String fullCommandRaw, List<String> args, Set<Character> flags) throws Exception {
        ...
        if (args.size() >= 4) gsa = Boolean.parseBoolean(args.get(6));
        if (args.size() >= 5) bda = Boolean.parseBoolean(args.get(7));

        double total = 0;

        for (int i = Math.max(1, current); i < max; i++) {
            // , cp && i >= Projects.URBAN_PLANNING.requiredCities(),
            //                    acp && i >= Projects.ADVANCED_URBAN_PLANNING.requiredCities(),
            //                    mp && i >= Projects.METROPOLITAN_PLANNING.requiredCities(),
            total += PW.City.nextCityCost(i, manifest, gsa, bda);
        }
