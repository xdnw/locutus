...
public class WarRoomCmd extends Command {
    ...
    public String onCommand(Guild guild, IMessageIO channel, User author, DBNation me, String fullCommandRaw, List<String> args, Set<Character> flags) throws Exception {
        ...
        if (arg.startsWith("https://docs.google.com/spreadsheets/") || arg.startsWith("sheet:")) {
            ...
            channel.sendMessage("Generating channels...", RateLimitedSources.COMMAND_RESULT);
            ...
            for (Map.Entry<DBNation, Set<DBNation>> entry : targets.entrySet()) {
                ...
                try {
                    if (args.get(1).length() > 1 && !args.get(1).equalsIgnoreCase("null")) {
                        RateLimitUtil.queue(warChan.sendMessage(args.get(1)), RateLimitedSources.COMMAND_RESULT);
                    }
        ...
        DBNation target = DiscordUtil.parseNation(arg, true, guild);
        if (target == null) return "Invalid target: `" + args.get(0) + "`";
        Set<DBNation> attackers = new ObjectLinkedOpenHashSet<>();
        for (int i = 1; i < args.size(); i++) {
            DBNation attacker = DiscordUtil.parseNation(args.get(i), true, guild);
            if (attacker == null) {
