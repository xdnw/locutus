...
//    }
    ...
    public static void addMenuButton(@Me IMessageIO io, @Me JSONObject cmdJson, @Me GuildDB db, @Me User user, AppMenu menu, @MenuLabel String label, @Default String command, @Switch("f") boolean force) {
        ...
        if (contains != null) {
            if (!force) {
                io.create().confirmation("Button already exists", "A button with the label `" + label + "` already exists. Do you want to overwrite it?", cmdJson).send(RateLimitedSources.COMMAND_RESULT);
                return;
            }
