...
public class PWBindings extends BindingHelper {
    ...
    public static List<ResourceType> rssTypes(String input) {
        ...
        for (String arg : input.split(",")) {
            ...
            if (remove) toAddOrRemove.forEach(types::remove);
    ...
    public static DBAlliancePosition position(@Me GuildDB db, @Me DBNation nation, String name) {
        ...
        if (alliances == null || alliances.isEmpty())
            throw new IllegalArgumentException("No alliances are set. See: " + CM.settings.cmd.toSlashMention() + " with key " + GuildDB.Key.ALLIANCE_ID);
        ...
        if (aaId != null && !alliances.contains(aaId))
            throw new IllegalArgumentException("Alliance " + aaId + " is not in the list of alliances registered to this guild: " + StringMan.getString(alliances.getIds()));
    ...
    public CityBuild city(@Me DBNation nation, @TextArea String input) {
        ...
        if (json != null) {
            ...
            json = build2.toString().replace("}", "") + "," + Objects.requireNonNull(build).toString().replace("{", "");
    ...
    public Map<ResourceType, Double> resources(String resources) {
        return PnwUtil.parseResources(resources);
    }
    ...
    public WarCategory warChannelBinding(@Me GuildDB db) {
        ...
        if (warChannel == null)
            throw new IllegalArgumentException("War channels are not enabled. " + CM.settings.cmd.create(GuildDB.Key.ENABLE_WAR_ROOMS.name(), "true", null, null).toSlashMention() + "");
        return warChannel;
    }
