...
//            factor = Math.min(factor, postiveTotal / (negativeTotal + postiveTotal));
    ...
    public String trending(@Me IMessageIO channel, @Me GuildDB db,
        ...
        sheet.attach(channel.create(), "trending").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
...
//        }
    ...
    public String tradeSubs(@Me User author, @Me IMessageIO io) {
        ...
        for (ResourceType type : ResourceType.values) {
            ...
            io.create().embed(title, body.toString()).commandButton(unsubCommand, emoji).send(RateLimitedSources.COMMAND_RESULT);
        }
