...
//        String[] lines = markdown.split("\n");
    ...
    public String view_document(PWGPTHandler handler, @Me IMessageIO io, @Me GuildDB db, EmbeddingSource source, @Switch("s") SpreadSheet sheet) throws GeneralSecurityException, IOException {
        ...
        sheet.attach(io.create(), "embeddings", null, false, 0).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
...
//    }
    ...
    public void emojifyChannels(PWGPTHandler gpt, @Me JSONObject command, @Me GuildDB db, @Me Guild guild,
        ...
        for (Map.Entry<Long, String> entry : rename.entrySet()) {
            ...
            if (desc != null) {
                RateLimitUtil.queue(channel.getManager().setTopic(desc), RateLimitedSources.COMMAND_RESULT);
            }
        }
