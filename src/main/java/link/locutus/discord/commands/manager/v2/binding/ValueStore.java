...
public interface ValueStore {
    ...
    default <V> Parser<V> addLazyProvider(Key<V> key, Supplier<V> provider) {
        return addParser(key, new SupplierProviderParser<>(key, provider));
    }

    default <V> Parser<V> addLazyProvider(Class<V> clazz, Supplier<V> provider) {
        return addLazyProvider(Key.of(clazz), provider);
    }
