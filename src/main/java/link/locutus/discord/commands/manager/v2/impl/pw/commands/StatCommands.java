...
public class StatCommands {
    ...
    public static String warCost(@Me @Default User author, @Me @Default Guild guild, @Me @Default GuildDB db,
        ...
        if (reimbursement != null) {
            if (db.hasReimbursedWar(war.warId)) {
                channel.send(alreadyReimbursedMessage(war, reimbursement), RateLimitedSources.COMMAND_RESULT);
    ...
    public String continent(@Me IMessageIO io) {
        ...
        msg.writeTable("Radiation", radsTable, false, footer).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
...
//    }
    ...
    public void allianceMetricByTurn(@Me @Default GuildDB db, @Me IMessageIO channel,
        ...
        msg.append("Done!" + (user != null ? " " + user.getAsMention() : "")).send(RateLimitedSources.COMMAND_RESULT);
    }
    ...
    public static String WarCostByResourceSheet(@Me IMessageIO channel, @Me JSONObject command, @Me @Default GuildDB db,
        ...
        sheet.attach(channel.create(), "war_cost_rss").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public static String WarCostByAllianceSheet(@Me IMessageIO channel, @Me @Default GuildDB guildDb,
        ...
        sheet.attach(channel.create(), "war_cost_aa").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public static String militaryRanking(@Me @Default GuildDB db, @Me IMessageIO channel,
        ...
        sheet.attach(msg, "alliance_ranking").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public void listMerges(
        ...
        dumper.iterateAll(Predicates.alwaysTrue(), (h, r) -> {
            ...
            if (start.get() + 5000 < now) {
                start.set(now);
                io.updateOptionally(msg, "Processing day " + aLong + "/" + (currentDay + 1), RateLimitedSources.COMMAND_PROGRESS);
            }
        });;
...
//        System.out.println("Alliance " + conflictManager.getAllianceName(aaId) + "\t" + aaId + "\t" + PW.getMarkdownUrl(aaId, true) + "\t" + new Date(TimeUtil.getTimeFromDay(day)) + "\t" + prevMembers + "\t" + members + "\t" + diff + "\t" + MathMan.format(100d * diff / prevMembers) + "%");
        ...
        sheet.updateClearCurrentTab();
        sheet.updateWrite();
        sheet.attach(io.create(), "merges").send(RateLimitedSources.COMMAND_RESULT);
    }
