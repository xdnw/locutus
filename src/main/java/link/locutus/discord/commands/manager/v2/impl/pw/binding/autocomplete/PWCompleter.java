...
public class PWCompleter extends BindingHelper {
        ...
        try {
            {
                ...
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> StringMan.autocompleteCommaEnum(SpyCount.Operation.class, input.toString(), OptionData.MAX_CHOICES))));
            }
            {
                Type type = getClass().getDeclaredField("AUDIT_TYPES_KEY").getGenericType();
                Key key = Key.of(type, Autocomplete.class);
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> StringMan.autocompleteCommaEnum(IACheckup.AuditType.class, input.toString(), OptionData.MAX_CHOICES))));
            }
            {
                Type type = getClass().getDeclaredField("CONTINENT_TYPES_KEY").getGenericType();
                Key key = Key.of(type, Autocomplete.class);
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> StringMan.autocompleteCommaEnum(Continent.class, input.toString(), OptionData.MAX_CHOICES))));
            }
            {
                Type type = getClass().getDeclaredField("SPYCOUNT_OPERATIONS_KEY").getGenericType();
                Key key = Key.of(type, Autocomplete.class);
            ...
            }
            {
                ...
                Key key = Key.of(type, Autocomplete.class);
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> StringMan.autocompleteCommaEnum(WarStatus.class, input.toString(), OptionData.MAX_CHOICES))));
            }
            {
                Type type = getClass().getDeclaredField("WARTYPES_KEY").getGenericType();
                Key key = Key.of(type, Autocomplete.class);
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> StringMan.autocompleteCommaEnum(WarType.class, input.toString(), OptionData.MAX_CHOICES))));
            }
            {
                Type type = getClass().getDeclaredField("ATTACKTYPES_KEY").getGenericType();
                Key key = Key.of(type, Autocomplete.class);
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> StringMan.autocompleteCommaEnum(AttackType.class, input.toString(), OptionData.MAX_CHOICES))));
            }
            {
            ...
            }
            {
                ...
                Key key = Key.of(type, Autocomplete.class);
                addBinding(store -> store.addParser(key, new FunctionConsumerParser(key, (BiFunction<ValueStore, Object, Object>) (valueStore, input) -> {
                    List<String> options = Arrays.stream(MilitaryUnit.values).map(Enum::name).collect(Collectors.toList());
    ...
    public List<Map.Entry<String, String>> DBAlliancePosition(@Me GuildDB db, String input) {
        ...
        return options.stream().map(f -> Map.entry(f.getName(), f.getInputName())).collect(Collectors.toList());
    }
