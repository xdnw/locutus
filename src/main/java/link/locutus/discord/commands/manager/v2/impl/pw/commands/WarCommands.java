...
public class WarCommands {
        ...
        if (mode != NationMeta.BeigeAlertMode.NO_ALERTS) {
            for (Guild guild : Locutus.imp().getDiscordApi().getMutualGuilds(user)) {
                ...
                if (role != null && member != null && member.getUnsortedRoles().contains(role)) {
                    try {
                        RateLimitUtil.queue(guild.removeRoleFromMember(user, role), RateLimitedSources.COMMAND_RESULT);
    ...
    @Command(desc = "View and test that the current automatic beige alert settings allow for notification", viewable = true)
    @WhitelistPermission
    @RolePermission(value = {Roles.BEIGE_ALERT, Roles.BEIGE_ALERT_OPT_OUT}, any = true)
    @CoalitionPermission(Coalition.RAIDPERMS)
    public String testBeigeAlertAuto(@Me GuildDB db, @Me DBNation me, @Me Member member) {
        ...
        result.append("- require-role:`").append(role == null ? "N/A" : role.getName()).append("`=").append(member.getUnsortedRoles().contains(role)).append("\n");
        result.append("- optout:`").append(optOut == null ? "N/A" : optOut.getName()).append("`=").append(member.getUnsortedRoles().contains(optOut)).append("\n");
    ...
    @Command(desc = "List your current beige reminders", aliases = {"beigeReminders", "listBeigeReminders"}, viewable = true)
    @WhitelistPermission
    @CoalitionPermission(Coalition.RAIDPERMS)
    ...
    public String raid(@Me DBNation me, @Me @Default GuildDB db, @Me @Default User user, @Me IMessageIO channel,
        ...
        try {
            ...
            System.out.println("Raid command result: " + result);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
    ...
    public String canIBeige(@Me IMessageIO channel, @Me GuildDB db, @Me DBNation me,
        ...
        channel.send(explanation, RateLimitedSources.COMMAND_RESULT);
    ...
    @Command(desc = "Find nations to gather intel on (sorted by infra * days since last intel)", viewable = true)
    @RolePermission(Roles.MEMBER)
    ...
    public String cancelUnblockadeRequest(@Me DBNation me, @Me GuildDB db, @Me User author) {
        ...
        if (unblockadeChannel != null) {
            ...
            RateLimitUtil.queue(unblockadeChannel.sendMessage(response.toString()), RateLimitedSources.COMMAND_RESULT);
        }
...
//        TODO info about baiting beige and things you can do under a blockade
    ...
    @Command(desc = "Find nations blockading your allies\n" +
            "Allies with requests to have their blockade lifted are prioritized", viewable = true)
    @RolePermission(Roles.MEMBER)
...
//        if (nation.active_m() > 4880) numCities += 0.5;
    ...
    public String Counterspy(@Me IMessageIO channel, @Me GuildDB db,
        ...
        if (operations.isEmpty()) throw new IllegalArgumentException("Valid operations: " + StringMan.getString(Operation.values()));
        if (counterWith == null) {
        ...
        Operation[] opTypes = operations.toArray(new Operation[0]);
        for (DBNation nation : counterWith) {
            ...
            if (opTypes.length == 1 && opTypes[0] == Operation.SPIES && enemySpies == 0) {
                return "Enemy has no spies";
            }

            Map.Entry<Operation, Map.Entry<Integer, Double>> best = SpyCount.getBestOp(mySpies, enemy, nation.hasProject(Projects.SPY_SATELLITE), opTypes);
            if (best != null) {
    ...
    public String raidSheet(ValueStore store, @Me IMessageIO io, @Me GuildDB db, @Me @Default User author,
        ...
        sheet.send(io, header.isEmpty() ? null : header.toString(), author == null ? null : author.getAsMention()).send(RateLimitedSources.COMMAND_RESULT);
...
//        enemies.removeIf(f -> f.getCities() < 4);
        ...
        sheet.attach(io.create(), "spy_intel").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String convertDtCSpySheet(@Me IMessageIO io, @Me @Default GuildDB db, @Me @Default User author, SpreadSheet input, @Switch("s") SpreadSheet output,
        ...
        output.send(io, null, warningStr + (author != null ? author.getAsMention() : null)).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String convertHidudeSpySheet(@Me IMessageIO io, @Me @Default GuildDB db, @Me @Default User author, SpreadSheet input, @Switch("s") SpreadSheet output,
        ...
        output.send(io, null, author == null ? null : author.getAsMention()).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String convertTKRSpySheet(@Me IMessageIO io, @Me @Default GuildDB db, @Me @Default User author, SpreadSheet input, @Switch("s") SpreadSheet output,
        ...
        String warningStr = warnings.isEmpty() ? "" : String.join("\n", warnings) + "\n";
        output.send(io, null, warningStr + (author == null ? null : author.getAsMention())).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String SpySheet(@Me IMessageIO io, @Me @Default User author, @Me @Default GuildDB db,
                           ...
                           @Arg(value = "Allowed spy operations", group = 0)
                               @Default("nuke,missile,ships,aircraft,tanks,spies") Set<Operation> allowedTypes,
        ...
        sheet.send(io, null, author == null ? null : author.getAsMention()).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
...
//                attStr += "|" + spyop.operation.name() + "|" + safety + "|" + spyop.spies;
    ...
    public String DepositSheetDate(@Me IMessageIO io, @Me @Default GuildDB db, Set<DBNation> nations,
        ...
        sheet.attach(io.create(), "activity").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
...
//        Map<Integer, Integer> lastSpyCounts = Locutus.imp().getNationDB().getLastSpiesByNation(nationIds, dayCutoff);
        ...
        sheet.attach(io.create(), "mmr", response).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public static String DeserterSheet(@Me IMessageIO io, @Me @Default GuildDB db, Set<DBAlliance> alliances,
        ...
        sheet.updateClearCurrentTab();
        sheet.updateWrite();
        sheet.attach(io.create(), "deserter").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String combatantSheet(@Me IMessageIO io, @Me @Default GuildDB db, Set<DBNation> nations,
        ...
        try {
            ...
            sheet.attach(io.create(), "combatant").send(RateLimitedSources.COMMAND_RESULT);
            return null;
        } catch (Throwable e) {
            e.printStackTrace();
            return null;
    ...
    @Command(desc = "List war rooms for an ally or enemy", viewable = true)
    ...
    @Command(desc = "Create war rooms from a blitz sheet", viewable = true)
    @RolePermission(Roles.MILCOM)
    public String warRoomSheet(@Me WarCategory warCat,
        ...
        msg.append("Creating channels...").send(RateLimitedSources.COMMAND_RESULT);
                ...
                if (roomChan != null) {
                    ...
                    try {
                        if (customMessage != null) {
                            RateLimitUtil.queue(roomChan.sendMessage(customMessage), RateLimitedSources.COMMAND_RESULT);
                        }
...
//                    if (op.operation == SpyCount.Operation.NUKE && op.defender.getNukes() <= 4) missileNuke++;
        ...
        msg.append("Done, sent " + sent + " messages" + (errorMsg.isEmpty() ? " (no errors)" : " (with errors)")).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String warSheet(@Me IMessageIO io, @Me @Default GuildDB db,
        ...
        sheet.attach(io.create(), "wars").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
...
//            warCat.sync();
        ...
        sheet.attach(io.create(), "counter").send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }

    @Command(desc = "Show a war card embed on discord for a war by id", viewable = true)
    public String warcard(@Me IMessageIO channel, int warId) throws IOException {
        new WarCard(warId).embed(channel, true, false);
        return null;
    }

    @Command(desc="Show war info for a nation", aliases = {"wars", "warinfo"}, viewable = true)
    @UserCommand
    public String wars(@Me IMessageIO channel, DBNation nation) {
        Set<DBWar> wars = nation.getActiveWars();
        String title = wars.size() + " wars";
        IShrink body = nation.getWarInfoEmbed();
        channel.create().embed(new EmbedShrink().setTitle(title).append(body)).send(RateLimitedSources.COMMAND_RESULT);
        return null;
    }
    ...
    public String spies(@Me DBNation me, DBNation nation,
                        ...
                        @Default() Safety requiredSafety) throws IOException {
        ...
        for (Operation op : Operation.values()) {
            Map.Entry<Operation, Map.Entry<Integer, Double>> best = SpyCount.getBestOp(60, nation, minSafety, maxSafety, me.hasProject(Projects.SPY_SATELLITE), op);
        ...
        if (!force) {
            for (DBNation attacker : attackersSorted) {
                if (!tracked.contains(attacker.getAlliance_id())) {
                    channel.create().confirmation("Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() + " is not an ally.", command).send(RateLimitedSources.COMMAND_RESULT);
                    return null;
                }
...
//                    DiscordUtil.pending(channel, message, "Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() + " is outside war range (see " + CM.nation.score.cmd.toSlashMention() + "). ", 'f');
                    channel.create().confirmation("Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() + " is outside war range (see " + CM.nation.score.cmd.toSlashMention() + "). ", command).send(RateLimitedSources.COMMAND_RESULT);
                    return null;
                }
                if (attacker.getOff() >= attacker.getMaxOff() && !allowAttackersWithMaxOffensives) {
                    channel.create().confirmation("Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() +  " already has max offensives. ", command).send(RateLimitedSources.COMMAND_RESULT);
                    return null;
                }
                if (attacker.getVm_turns() > 0) {
                    channel.create().confirmation( "Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() + " is in VM. ", command).send(RateLimitedSources.COMMAND_RESULT);
                    return null;
                }
                if (attacker.isGray() && attacker.active_m() > 1440 || attacker.getCities() < 10 && attacker.active_m() > 2000) {
                    channel.create().confirmation( "Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() + " is gray/inactive. ", command).send(RateLimitedSources.COMMAND_RESULT);
                    return null;
                }
                if (attacker.getNumWars() > 0 && attacker.getRelativeStrength() < 1) {
                    channel.create().confirmation( "Error: Unsuitable counter", attacker.getNationUrlMarkup() + " | " + attacker.getAllianceUrlMarkup() + " is already involved in heavy conflict.", command).send(RateLimitedSources.COMMAND_RESULT);
                    return null;
                }
            }
        }
    ...
    public String warcat(@Me WarCategory warCat, @Me WarRoom waRoom, @Me TextChannel channel,
        ...
        RateLimitUtil.complete(channel.getManager().setParent(category), RateLimitedSources.COMMAND_RESULT);
    ...
    @Command(desc = "Generate a list of possible alliance blitz targets (for practice)\n" +
            "Target alliances are sorted by free war slots", aliases = {"blitzpractice","blitztargets"}, viewable = true)
    public String BlitzPractice(@Me GuildDB db,
        ...
        channel.create().paginate(title, command, page, perPage, IShrink.toList(results)).send(RateLimitedSources.COMMAND_RESULT);
    ...
    public void warRange(@Me IMessageIO io, double score) {
        ...
        io.create().embed(title, sb.toString()).send(RateLimitedSources.COMMAND_RESULT);
    }
}
