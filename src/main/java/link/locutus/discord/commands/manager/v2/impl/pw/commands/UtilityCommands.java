...
import link.locutus.discord.util.task.roles.AutoRoleIssue;
import link.locutus.discord.util.task.roles.AutoRoleIssueType;
import link.locutus.discord.util.task.roles.AutoRoleMaskedMember;
import link.locutus.discord.util.task.roles.AutoRoleMemberResult;
import link.locutus.discord.util.task.roles.AutoRoleResult;
import link.locutus.discord.util.task.roles.AutoRoleService;
import link.locutus.discord.util.task.roles.AutoRoleSyncState;
...
public class UtilityCommands {
    ...
    public String calculateColorRevenue(@Me GuildDB db, @Me IMessageIO io,
        ...
        for (Map.Entry<NationColor, Set<DBNation>> entry : changeColors.entrySet()) {
            ...
            for (DBNation nation : entry.getValue()) {
                if (nation.getColor() == entry.getKey()
                        || ((nation.isGray() || nation.isBeige()) && entry.getKey() == NationColor.GRAY))
                    continue;
        ...
        for (DBNation nation : Locutus.imp().getNationDB()
                .getNationsMatching(f -> f.getAlliance_id() != 0 && f.getVm_turns() == 0)) {
        ...
        // int nations, int nationsBelowC21, double aggregateDNR, double averageDNR, int
        // growthTurnBonus, int growthTurnBonusUncapped, int recruitTurnBonus
        ...
        for (NationColor color : NationColor.values()) {
            NationColor.TurnBonusInfo oldBonus = color.getTurnBonus(oldNationsMap, oldCapInfo.cap(),
                    oldNationsBelowC21);
            NationColor.TurnBonusInfo newBonus = color.getTurnBonus(newNationsMap, newCapInfo.cap(),
                    newNationsBelowC21);
            ...
            if (oldTotal != newTotal) {
                ...
                lines.append("$" + MathMan.format(oldTotal) + " -> $" + MathMan.format(newTotal) + " (" + signSym + "$"
                        + MathMan.format(Math.abs(newTotal - oldTotal)) + ")");
            } else {
            ...
            if (oldNationsOnColor != newNationsOnColor) {
                ...
                lines.append(MathMan.format(oldNationsOnColor) + " -> " + MathMan.format(newNationsOnColor) + " ("
                        + signSym + MathMan.format(Math.abs(newNationsOnColor - oldNationsOnColor)) + ")");
            } else {
            ...
            if (oldNationsBelowC21OnColor != newNationsBelowC21OnColor) {
                ...
                lines.append(MathMan.format(oldNationsBelowC21OnColor) + " -> "
                        + MathMan.format(newNationsBelowC21OnColor) + " (" + signSym
                        + MathMan.format(Math.abs(newNationsBelowC21OnColor - oldNationsBelowC21OnColor)) + ")");
            } else {
                lines.append(MathMan.format(oldNationsBelowC21OnColor));
            }
            lines.append("\n");
            ...
            if (oldBonus.growthTurnBonus() != newBonus.growthTurnBonus()) {
                ...
                lines.append(MathMan.format(oldBonus.growthTurnBonus()) + " -> "
                        + MathMan.format(newBonus.growthTurnBonus()) + " (" + signSym
                        + MathMan.format(Math.abs(newBonus.growthTurnBonus() - oldBonus.growthTurnBonus())) + ")");
            } else {
                lines.append(MathMan.format(oldBonus.growthTurnBonus()));
            }
            lines.append("\n");
            ...
            if (oldBonus.recruitTurnBonus() != newBonus.recruitTurnBonus()) {
                ...
                lines.append(MathMan.format(oldBonus.recruitTurnBonus()) + " -> "
                        + MathMan.format(newBonus.recruitTurnBonus()) + " (" + signSym
                        + MathMan.format(Math.abs(newBonus.recruitTurnBonus() - oldBonus.recruitTurnBonus())) + ")");
            } else {
                lines.append(MathMan.format(oldBonus.recruitTurnBonus()));
            }
            lines.append("\n");
    ...
    public String channelCount(@Me IMessageIO channel, @Me Guild guild, @Switch("m") boolean list_added_members,
            @Switch("r") boolean list_added_roles) {
            ...
            for (GuildChannel catChannel : category.getChannels()) {
                ...
                if (catChannel instanceof TextChannel msgChan) {
                    if (list_added_members) {
                        List<Member> members = msgChan.getMemberPermissionOverrides().stream()
                                .map(PermissionOverride::getMember).filter(Objects::nonNull)
                        ...
                        if (!members.isEmpty()) {
                            ...
                            for (Member member : members) {
                                ...
                                if (nation != null) {
                                    channelList
                                            .append(" (" + nation.getNation() + " | " + nation.getAllianceName() + ")");
                                }
                                channelList.append(", ");
                            }
                    ...
                    if (list_added_roles) {
                        List<Role> roles = msgChan.getRolePermissionOverrides().stream()
                                .map(PermissionOverride::getRole).filter(Objects::nonNull).collect(Collectors.toList());
                        if (!roles.isEmpty()) {
    ...
    // @WhitelistPermission
    // public String findSpyOp(@Me DBNation me, String times, int defenderSpies,
    // @Default DBNation defender) {
    ...
    // List<DBSpyUpdate> updates =
    // Locutus.imp().getNationDB().getSpyActivity(timestamp, interval);
    // for (DBSpyUpdate update : updates) {
    //// nations.put(update.nation_id, nations.getOrDefault(update.nation_id, 0) +
    // 1);
    ...
    // !attacker.isInSpyRange(defender)) || attacker.getNation_id() ==
    // defender.getNation_id()) continue;
    //
    ...
    //
    // double odds = SpyCount.getOdds(spiesUsed, defenderSpies, safety,
    // SpyCount.Operation.SPIES, defender);
    ...
    // }
    //
    // List<Map.Entry<DBNation, Map.Entry<Double, String>>> sorted = new
    // ArrayList<>(allOdds.entrySet());
    // sorted.sort((o1, o2) -> Double.compare(o2.getValue().getKey(),
    // o1.getValue().getKey()));
    //
    // if (sorted.isEmpty()) {
    // return "No results";
    ...
    public String listOffshores() {
        ...
        Map<Integer, List<DBNation>> map = Locutus.imp().getNationDB().getNationsByAlliance(false, false, true, true,
                true);
        outer: for (Map.Entry<Integer, List<DBNation>> entry : map.entrySet()) {
            ...
            for (DBWar war : Locutus.imp().getWarDb().getWarsByAlliance(aa.getAlliance_id())) {
                ...
                int lostAA = war.getStatus() == WarStatus.ATTACKER_VICTORY ? war.getDefender_aa()
                        : war.getStatus() == WarStatus.DEFENDER_VICTORY ? war.getAttacker_aa() : 0;
                boolean isLooted = lostAA != 0 && lostAA == aaid;
                if (!isLooted)
                    continue;
                int otherAA = war.getStatus() == WarStatus.ATTACKER_VICTORY ? war.getAttacker_aa()
                        : war.getStatus() == WarStatus.DEFENDER_VICTORY ? war.getDefender_aa() : 0;
                if (otherAA == aaid)
                    continue;
                ...
                if (lowMil) {
                    for (DBNation member : members) {
                        if (member.daysSinceLastOffensive() < 30)
                            lowMil = false;
                    }
                    if (lowMil) {
                        continue outer;
            ...
            List<String> treaties = aa.getTreaties().keySet().stream().map(f -> PW.getName(f, true))
                    .collect(Collectors.toList());
            if (!treaties.isEmpty()) {
    ...
    public String markAsOffshore(@Me User author, @Me DBNation me, DBAlliance offshore, DBAlliance parent) {
        if (!Roles.ADMIN.hasOnRoot(author)) {
            ...
            if (expectedParent != parent) {
                ...
                if (me.getPositionEnum().id < Rank.OFFICER.id)
                    return "You are not leader in " + offshore.getName();
                ...
                if (db != null) {
                    ...
                    if (member == null)
                        return "You are not member in " + guild;
                    if (member.hasPermission(Permission.ADMINISTRATOR))
                        return "You are not admin in: " + guild;
                }
            }
        }
    ...
    public String findOffshores(@Timestamp long cutoff, Set<DBAlliance> enemiesList,
            @Default() Set<DBAlliance> alliesList) {
        if (alliesList == null)
            alliesList = Collections.emptySet();
        ...
        Map<Integer, Set<AllianceChange>> removesByNation = removes.entrySet().stream()
                .flatMap(f -> f.getValue().stream())
                .collect(Collectors.groupingBy(AllianceChange::getNationId, Collectors.toSet()));
        ...
        Map<Integer, Map.Entry<Integer, Map.Entry<Integer, Double>>> offshoresTransfers = new HashMap<>(); // Ofshore AA
                                                                                                           // id ->
                                                                                                           // (parent AA
                                                                                                           // id, num
                                                                                                           // transfers,
        ...
        Map<Integer, List<DBNation>> alliances = Locutus.imp().getNationDB().getNationsByAlliance(false, false, true,
                true, true);
        outer: for (Map.Entry<Integer, List<DBNation>> entry : alliances.entrySet()) {
            int aaId = entry.getKey();
            if (allies.contains(aaId) || enemies.contains(aaId))
                continue;
            ...
            for (DBAlliance treaty : treaties) {
                if (allies.contains(treaty.getAlliance_id()))
                    continue outer;
            }
            ...
            if (!transfers.isEmpty()) {
                ...
                for (Transaction2 tx : transfers) {
                    if (tx.banker_nation == tx.receiver_id)
                        continue;
            ...
            for (DBNation nation : nations) {
                ...
                Set<AllianceChange> nationRemoves = removesByNation.getOrDefault(nation.getNation_id(),
                        Collections.emptySet());
                for (AllianceChange change : nationRemoves) {
                    int previousAA = change.getFromId();
                    if (!enemies.contains(previousAA))
                        continue;
                    if (change.getDate() < cutoff)
                        continue;
...

        {
            ...
            if (!offshoresWar.isEmpty()) {
                ...
                for (Map.Entry<Integer, Integer> entry : offshoresWar.entrySet()) {
                    response.append(PW.getName(entry.getKey(), true) + " " + getName.apply(entry.getKey())
                            + " attacking " + PW.getName(entry.getValue(), true) + "\n");
                }
                response.append("\n");
            }
            ...
            if (!offshoresTreaty.isEmpty()) {
                ...
                for (Map.Entry<Integer, Integer> entry : offshoresTreaty.entrySet()) {
                    response.append(PW.getName(entry.getKey(), true) + " <" + getName.apply(entry.getKey())
                            + "> treatied " + PW.getName(entry.getValue(), true) + "\n");
                }
                response.append("\n");
            }
            ...
            if (!offshoresOfficer.isEmpty()) {
                ...
                for (Map.Entry<Integer, Integer> entry : offshoresOfficer.entrySet()) {
                    response.append(PW.getName(entry.getKey(), true) + " <" + getName.apply(entry.getKey())
                            + "> formerly officer in " + PW.getName(entry.getValue(), true) + "\n");
                }
                response.append("\n");
            }
            ...
            if (!offshoresMember.isEmpty()) {
                ...
                for (Map.Entry<Integer, Integer> entry : offshoresMember.entrySet()) {
                    response.append(PW.getName(entry.getKey(), true) + " <" + getName.apply(entry.getKey())
                            + "> formerly member in " + PW.getName(entry.getValue(), true) + "\n");
                }
                response.append("\n");
            }
            ...
            if (!offshoresTransfers.isEmpty()) {
                ...
                for (Map.Entry<Integer, Map.Entry<Integer, Map.Entry<Integer, Double>>> entry : offshoresTransfers
                        .entrySet()) {
                    ...
                    response.append(PW.getName(entry.getKey(), true) + " <" + PW.getAllianceUrl(entry.getKey())
                            + "> has " + num + " transfers with " + PW.getName(otherAAId, true) + " worth ~$"
                            + MathMan.format(value) + "\n");
                }
            }
            if (response.length() == 0)
                return "No results founds in the specified timeframe";
    ...
    public static String nap(@Default("false") boolean listExpired) {
        ...
        naps.put(238332L * 7200 * 1000,
                "<https://politicsandwar.fandom.com/wiki/Blue_Balled>\n<https://forum.politicsandwar.com/index.php?/topic/36719-peace-all-in-a-day/>");
        naps.put(239436L * 7200 * 1000,
                "<https://forum.politicsandwar.com/index.php?/topic/39524-treaty-why-nap-when-you-can-sleep/>");
        naps.put(1726938000000L,
                "(Mid-turn, 5:00pm UTC) <https://forum.politicsandwar.com/index.php?/topic/41182-peace-casino-royale/>");
        naps.put(1743120000000L,
                "<https://forum.politicsandwar.com/index.php?/topic/46146-peace-darkest-hour-deux-done/>");
        naps.put(1737763200000L,
                "<https://forum.politicsandwar.com/index.php?/topic/46146-peace-darkest-hour-deux-done/>");
        ...
        naps.put(1755129600000L,
                "<https://forum.politicsandwar.com/index.php?/topic/49670-peace-quit-monkeyin%E2%80%99-around/>");
        naps.put(1756339200000L,
                "<https://forum.politicsandwar.com/index.php?/topic/49931-doe-peace-introducing%E2%80%A6-tulip/>");
        naps.put(1760572800000L,
                "<https://forum.politicsandwar.com/index.php?/topic/52693-%E2%80%9Cwhen-in-rome%E2%80%A6%E2%80%9D/>");
    ...
    public String warRanking(@Me JSONObject command, @Me IMessageIO channel, @Timestamp long time,
            @AllowDeleted Set<NationOrAlliance> attackers, @AllowDeleted Set<NationOrAlliance> defenders,
            @Arg("Only include offensive wars in the ranking") @Switch("o") boolean onlyOffensives,
            @Arg("Only include defensive wars in the ranking") @Switch("d") boolean onlyDefensives,
            @Switch("c") @Arg("Only rank the `attackers` side") boolean only_rank_attackers,
            @Arg("Rank the average wars per alliance member") @Switch("n") boolean normalizePerMember,
            @Arg("Ignore inactive nations when determining alliance member counts") @Switch("i") boolean ignore2dInactives,
    ...
    @Command(desc = "Calculate the costs of purchasing cities (from current to max)", aliases = { "citycost",
            "citycosts" }, viewable = true)
    public String CityCost(@Range(min = 1, max = 100) int currentCity, @Range(min = 1, max = 100) int maxCity,
        ...
        if (maxCity > 1000)
            throw new IllegalArgumentException("Max cities 1000");
    ...
    @Command(desc = "Calculate the costs of purchasing infra (from current to max)", aliases = { "InfraCost",
            "infrastructurecost", "infra", "infrastructure", "infracosts" }, viewable = true)
    public String InfraCost(@Range(min = 0, max = 40000) int currentInfra, @Range(min = 0, max = 40000) int maxInfra,
        ...
        if (maxInfra > 40000)
            throw new IllegalArgumentException("Max infra 40000");
    ...
    @Command(desc = "Calculate the costs of purchasing land (from current to max)", aliases = { "LandCost", "land",
            "landcosts" }, viewable = true)
    public String LandCost(@Range(min = 0, max = 40000) int currentLand,
            ...
            @Switch("c") @Default("1") int cities) {
        if (maxLand > 40000)
            throw new IllegalArgumentException("Max land 40000");
    ...
    @Command(desc = "Calculate the score of various things. Each argument is option, and can go in any order", viewable = true, groups = {
            "Nation Details",
            "Military Units",
    ...
    public String score(
        ...
        if (soldiers != null)
            nation.setSoldiers(soldiers);
        if (tanks != null)
            nation.setTanks(tanks);

        if (aircraft != null)
            nation.setAircraft(aircraft);
        if (ships != null)
            nation.setShips(ships);
        if (missiles != null)
            nation.setMissiles(missiles);
        if (nukes != null)
            nation.setNukes(nukes);
        if (projects != null) {
            if (projects >= Projects.values.length)
                return "Too many projects: " + projects;
        ...
        if (builtMMR != null) {
            nation.setMMR((builtMMR.getBarracks()), (builtMMR.getFactory()), (builtMMR.getHangar()),
                    (builtMMR.getDrydock()));
        }
        ...
        for (int i = types.length - 1; i >= 0; i--) {
            ...
            if (value == null || ArrayUtil.toCents(value) == 0)
                continue;
        ...
        return md.toString() +
                ...
                + "\n" +
                "Can be Attacked By: " + MathMan.format(score / PW.WAR_RANGE_MAX_MODIFIER) + "- "
                + MathMan.format(score / 0.75) + "\n" +
                "Spy range: " + MathMan.format(score * 0.4) + "- " + MathMan.format(score * 1.5);
    }

    @Command(desc = "Check how many turns are left in the city/project timer", aliases = { "TurnTimer", "Timer",
            "CityTimer", "ProjectTimer" }, viewable = true)
    @UserCommand
    ...
    @Command(desc = "Check how many projects slots a nation has", aliases = { "ProjectSlots", "ProjectSlot",
            "projects" }, viewable = true)
    ...
    public void projectCostCsv(@Me IMessageIO io, @Me GuildDB db, @Switch("s") SpreadSheet sheet)
    ...
    public static String loot(@Me IMessageIO output, @Me @Default DBNation me, NationOrAlliance nationOrAlliance,
            @Arg("Score of the defeated nation\n" +
                    "i.e. For determining bank loot percent") @Default Double nationScore,
            @Arg("Loot with pirate war policy\n" +
                    "Else: Uses your war policy") @Switch("p") boolean pirate) {
        ...
        if (nationScore == null)
            nationScore = nationOrAlliance.isNation() ? nationOrAlliance.asNation().getScore()
                    : me == null ? 1 : me.getScore();
        DBAlliance alliance = nationOrAlliance.isAlliance() ? nationOrAlliance.asAlliance()
                : nationOrAlliance.asNation().getAlliance(false);
        ...
        if (alliance != null) {
            ...
            if (aaLootEntry != null) {
                ...
                extraInfo.add("Alliance Last looted: "
                        + TimeUtil.secToTime(TimeUnit.MILLISECONDS, System.currentTimeMillis() - date));
        ...
        if (nationOrAlliance.isNation()) {
            ...
            if (lootInfo != null) {
                ...
                info.append(
                        "(" + TimeUtil.secToTime(TimeUnit.MILLISECONDS, System.currentTimeMillis() - lootInfo.getDate())
                                + " ago)");
                info.append(", worth: $" + MathMan.format(originalValue) + "($" + MathMan.format(originalLootable)
                        + " lootable)");
                if (nationOrAlliance.asNation().active_m() > 1440)
                    info.append("- inactive for "
                            + TimeUtil.secToTime(TimeUnit.MINUTES, nationOrAlliance.asNation().active_m()));
        ...
        if (nationLoot != null) {
            response.append("Nation Loot (worth: $" + MathMan.format(ResourceType.convertedTotal(nationLoot)) + "): ```"
                    + ResourceType.toString(nationLoot) + "``` ");
            ResourceType.add(total, nationLoot);
        }
        if (allianceLoot != null) {
            response.append("Alliance Loot (worth: $" + MathMan.format(ResourceType.convertedTotal(allianceLoot))
                    + "): ```" + ResourceType.toString(allianceLoot) + "``` ");
            ResourceType.add(total, allianceLoot);
        }
        if (revenue != null) {
            response.append("Revenue (" + revenueTurns + " turns @" + MathMan.format(revenueFactor) + "x, worth: $"
                    + MathMan.format(ResourceType.convertedTotal(revenue)) + ") ```" + ResourceType.toString(revenue)
                    + "``` ");
            ResourceType.add(total, revenue);
        }
        boolean perScore = nationScore == 1;
        response.append("Total Loot " + (perScore ? "Per Score" : "") + " (worth: $"
                + MathMan.format(ResourceType.convertedTotal(total)) + "): ```" + ResourceType.toString(total)
                + "``` ");
        ...
        if (nationOrAlliance.isNation() && nationOrAlliance.asNation().active_m() > 1440
            ...
            Locutus.imp().getExecutor().submit(() -> {
                ...
                for (Map.Entry<LoginFactor, Double> entry : factors.getResult().entrySet()) {
                    ...
                    append.add("quit chance (" + factor.name + "=" + factor.toString(factor.get(nation)) + "): "
    ...
    public String ProjectCost(@Me @Default GuildDB db, @Me IMessageIO channel,
        ...
        if (sheet != null && nations == null)
            throw new IllegalArgumentException("You must specify `nations` for `sheet` option to be used");
        if ((ignoreProjectSlots || ignoreRequirements || ignoreProjectCity) && nations == null)
            throw new IllegalArgumentException("You must specify `nations` for `ignore` options to be used");
        ...
        if (nations == null) {
            ...
            if (projectsList.size() > 1) {
                response.append("Total:\n```" + ResourceType.toString(costs) + "```\nworth: ~$"
                        + MathMan.format(ResourceType.convertedTotal(costs)) + "\n");
            }
            return response.toString();
        } else {
            if (sheet == null) {
            ...
            for (DBNation nation : nations) {
                ...
                if (governmentSupportAgency)
                    nationCopy.setProject(Projects.GOVERNMENT_SUPPORT_AGENCY);
                if (domesticAffairs)
                    nationCopy.setProject(Projects.BUREAU_OF_DOMESTIC_AFFAIRS);
                ...
                for (Project project : projectsList) {
                    ...
                    if (nation.hasProject(project)) {
                        ...
                        errors.add("cities:" + project.requiredCities() + " < " + nation.getCities() + " for "
                                + project.name());
                    } else if (!ignoreProjectCity && nation.getCities() > project.maxCities()) {
                        errors.add("cities:" + project.maxCities() + " < " + nation.getCities() + " for "
                                + project.name());
                ...
                header.set(3,
                        nationCopy.getDomesticPolicy() == DomesticPolicy.TECHNOLOGICAL_ADVANCEMENT ? "true" : "false");
    ...
    public static String autoroleall(@Me GuildDB db, @Me IMessageIO channel, @Me JSONObject command,
        ...
        if (force) {
            return formatBulkAutoRoleExecution(result);
        }

        String body = autoRolePreviewHeader(result.sync, result.role_names);
        String resultStr = formatBulkAutoRolePreview(result);
        String separator = "\n\n------------\n\n";
        ...
        if (attachFile) {
            msg = msg.file("role_changes.txt", resultStr);
        }

        String maskedMembers = formatMaskedNonMembers(result.masked_non_members);
        if (!maskedMembers.isEmpty()) {
    ...
    public static String autorole(@Me GuildDB db, @Me IMessageIO channel, @Me JSONObject command, Member member,
            @Switch("f") boolean force) {
        AutoRoleResult result = AutoRoleService.autorole(db, member, force);
        if (containsIssue(result.result.issues, AutoRoleIssueType.NOT_REGISTERED)) {
            return formatIssues(result.result.issues, result.role_names);
        }
        if (force) {
            return formatSingleAutoRoleExecution(result);
        }
    ...
    private static String autoRolePreviewHeader(AutoRoleSyncState sync, Map<Long, String> roleNames) {
        StringBuilder body = new StringBuilder("`note: Results may differ if settings or users change`\n");
        String syncText = AutoRoleTextFormatter.formatSyncState(sync, roleNames);
        if (!syncText.isBlank()) {
            body.append(syncText);
        }
        return body.toString().trim();
    }

    private static String formatSingleAutoRolePreview(AutoRoleResult result) {
        StringBuilder response = new StringBuilder();
        appendCreateRoles(response, result.create_roles);
        appendRenameRoles(response, result.rename_roles, result.role_names, "Rename Roles");
        appendMemberPlan(response, result.result, result.role_names);
        return response.length() == 0 ? "No changes" : response.toString().trim();
    }

    private static String formatSingleAutoRoleExecution(AutoRoleResult result) {
        StringBuilder response = new StringBuilder();
        appendCreatedRoles(response, result.created_roles, result.role_names);
        appendRenameRoles(response, result.renamed_roles, result.role_names, "Renamed Roles");
        appendIssueBlock(response, "Execution Issues", result.execution_issues, result.role_names, false);
        appendMemberExecution(response, result.result, result.role_names);
        return response.length() == 0 ? "No changes" : response.toString().trim();
    }

    private static String formatBulkAutoRolePreview(AutoRoleBulkResult result) {
        StringBuilder response = new StringBuilder();
        appendCreateRoles(response, result.create_roles);
        appendRenameRoles(response, result.rename_roles, result.role_names, "Rename Roles");
        for (AutoRoleMemberResult memberResult : result.results) {
            appendMemberPlan(response, memberResult, result.role_names);
        }
        return response.length() == 0 ? "No changes" : response.toString().trim();
    }

    private static String formatBulkAutoRoleExecution(AutoRoleBulkResult result) {
        StringBuilder response = new StringBuilder();
        appendCreatedRoles(response, result.created_roles, result.role_names);
        appendRenameRoles(response, result.renamed_roles, result.role_names, "Renamed Roles");
        appendIssueBlock(response, "Execution Issues", result.execution_issues, result.role_names, false);
        for (AutoRoleMemberResult memberResult : result.results) {
            appendMemberExecution(response, memberResult, result.role_names);
        }
        return response.length() == 0 ? "No changes" : response.toString().trim();
    }

    private static void appendMemberPlan(StringBuilder response, AutoRoleMemberResult memberResult,
            Map<Long, String> roleNames) {
        if (!hasPlanContent(memberResult)) {
            return;
        }
        if (response.length() > 0) {
            response.append("\n");
        }
        response.append(formatMemberLabel(memberResult)).append("\n");
        appendInlineList(response, "  Create Roles",
                memberResult.create_roles.stream().map(UtilityCommands::formatAllianceLabel).toList());
        appendInlineList(response, "  Add Roles",
                memberResult.add_roles.stream().map(roleId -> formatRoleLabel(roleId, roleNames)).toList());
        appendInlineList(response, "  Remove Roles",
                memberResult.remove_roles.stream().map(roleId -> formatRoleLabel(roleId, roleNames)).toList());
        if (memberResult.clear_nickname) {
            response.append("  Clear Nickname: true\n");
        } else if (memberResult.nickname != null) {
            response.append("  Set Nickname: ").append(memberResult.nickname).append("\n");
        }
        appendIssueBlock(response, "  Issues", memberResult.issues, roleNames, true);
    }

    private static void appendMemberExecution(StringBuilder response, AutoRoleMemberResult memberResult,
            Map<Long, String> roleNames) {
        if (!hasExecutionContent(memberResult)) {
            return;
        }
        if (response.length() > 0) {
            response.append("\n");
        ...
        response.append(formatMemberLabel(memberResult)).append("\n");
        appendInlineList(response, "  Added Roles",
                memberResult.added_roles.stream().map(roleId -> formatRoleLabel(roleId, roleNames)).toList());
        appendInlineList(response, "  Removed Roles",
                memberResult.removed_roles.stream().map(roleId -> formatRoleLabel(roleId, roleNames)).toList());
        if (memberResult.cleared_nickname) {
            response.append("  Cleared Nickname: true\n");
        } else if (memberResult.applied_nickname != null) {
            response.append("  Applied Nickname: ").append(memberResult.applied_nickname).append("\n");
        }
        appendIssueBlock(response, "  Execution Issues", memberResult.execution_issues, roleNames, true);
    }

    private static void appendCreateRoles(StringBuilder response, List<Integer> createRoles) {
        ...
        response.append("Create Roles:\n");
        for (Integer allianceId : createRoles) {
            response.append("- ").append(formatAllianceLabel(allianceId)).append("\n");
        }
    }

    private static void appendCreatedRoles(StringBuilder response, List<Long> createdRoles,
            Map<Long, String> roleNames) {
        if (createdRoles == null || createdRoles.isEmpty()) {
            return;
        }
        if (response.length() > 0) {
            response.append("\n");
        }
        response.append("Created Roles:\n");
        for (Long roleId : createdRoles) {
            response.append("- ").append(formatRoleLabel(roleId, roleNames)).append("\n");
        }
    }

    private static void appendRenameRoles(StringBuilder response, Map<Long, String> renameRoles,
            Map<Long, String> roleNames, String title) {
        if (renameRoles == null || renameRoles.isEmpty()) {
            return;
        }
        if (response.length() > 0) {
            response.append("\n");
        ...
        renameRoles.forEach((roleId, newName) -> response.append("- ").append(formatRoleLabel(roleId, roleNames))
                .append(" -> ").append(newName).append("\n"));
    }

    private static void appendInlineList(StringBuilder response, String title, List<String> values) {
        if (values == null || values.isEmpty()) {
            return;
        }
        response.append(title).append(": ").append(String.join(", ", values)).append("\n");
    }

    private static void appendIssueBlock(StringBuilder response, String title, List<AutoRoleIssue> issues,
            Map<Long, String> roleNames, boolean alreadyIndented) {
        if (issues == null || issues.isEmpty()) {
            return;
        }
        if (!alreadyIndented && response.length() > 0) {
            response.append("\n");
        }
        response.append(title).append(":\n");
        for (AutoRoleIssue issue : issues) {
            response.append(alreadyIndented ? "  - " : "- ").append(formatIssue(issue, roleNames)).append("\n");
        }
    }

    private static String formatIssues(List<AutoRoleIssue> issues, Map<Long, String> roleNames) {
        if (issues == null || issues.isEmpty()) {
            return "No changes";
        }
        List<String> values = new ArrayList<>();
        for (AutoRoleIssue issue : issues) {
            values.add(formatIssue(issue, roleNames));
        }
        return String.join("\n", values);
    }

    private static boolean hasPlanContent(AutoRoleMemberResult result) {
    ...
    private static boolean hasExecutionContent(AutoRoleMemberResult result) {
        return !result.added_roles.isEmpty()
    ...
    private static boolean containsIssue(List<AutoRoleIssue> issues, AutoRoleIssueType type) {
        if (issues == null) {
            return false;
        }
    ...
    private static String formatIssue(AutoRoleIssue issue, Map<Long, String> roleNames) {
        if (issue == null || issue.type == null) {
            return "Unknown issue";
        }
        String roleLabel = issue.role_id == null ? null : formatRoleLabel(issue.role_id, roleNames);
        return switch (issue.type) {
            case NOT_REGISTERED -> "Not registered. See: " + CM.register.cmd.toSlashMention();
            case MISSING_REGISTERED_ROLE_MAPPING -> "No registered role mapping found. See: "
                    + CM.role.setAlias.cmd.locutusRole(Roles.REGISTERED.name()).discordRole(null);
            case MISSING_REGISTERED_ROLE ->
                roleLabel == null ? "Missing registered role" : "Missing registered role `" + roleLabel + "`";
            case AUTO_NICKNAME_DISABLED -> "Auto nickname is disabled";
            case PLANNING_FAILED -> formatDetail("Planning failed", issue);
            case CREATE_ROLE_FAILED ->
                formatDetail("Failed to create role for alliance " + formatAllianceLabel(issue.alliance_id), issue);
            case ADD_ROLE_FAILED -> formatDetail("Failed to add role `" + roleLabel + "`", issue);
            case REMOVE_ROLE_FAILED -> formatDetail("Failed to remove role `" + roleLabel + "`", issue);
            case RENAME_ROLE_FAILED -> formatDetail("Failed to rename role `" + roleLabel + "`", issue);
            case SET_NICKNAME_FAILED -> formatDetail("Failed to set nickname" + formatNickname(issue.nickname), issue);
            case CLEAR_NICKNAME_FAILED -> formatDetail("Failed to clear nickname", issue);
            case ROLE_ALREADY_PRESENT ->
                roleLabel == null ? "Member already has role" : "Member already has role `" + roleLabel + "`";
            case ROLE_NOT_PRESENT ->
                roleLabel == null ? "Member does not have role" : "Member does not have role `" + roleLabel + "`";
            case ROLE_ALREADY_NAMED -> roleLabel == null ? "Role already has the target name"
                    : "Role `" + roleLabel + "` already has the target name";
            case NICKNAME_ALREADY_SET -> "Nickname already set" + formatNickname(issue.nickname);
            case NICKNAME_NOT_PRESENT -> "Member does not have a nickname";
        };
    }

    private static String formatDetail(String prefix, AutoRoleIssue issue) {
        StringBuilder response = new StringBuilder(prefix);
        if (issue.error_type != null && !issue.error_type.isBlank()) {
            response.append(": ").append(issue.error_type);
    ...
    private static String formatRoleLabel(long roleId, Map<Long, String> roleNames) {
        String name = roleNames.get(roleId);
        return name == null ? Long.toString(roleId) : name + " (" + roleId + ")";
    }

    private static String formatAllianceLabel(Integer allianceId) {
        if (allianceId == null) {
            return "unknown";
        }
        DBAlliance alliance = DBAlliance.get(allianceId);
        return alliance == null ? Integer.toString(allianceId) : allianceId + " " + alliance.getName();
    }

    private static String formatMemberLabel(AutoRoleMemberResult memberResult) {
        return memberResult.display_name + " (<@" + memberResult.user_id + ">)";
    }

    private static String formatMaskedNonMembers(List<AutoRoleMaskedMember> maskedMembers) {
        if (maskedMembers == null || maskedMembers.isEmpty()) {
            return "";
        }

        StringBuilder response = new StringBuilder();
        for (AutoRoleMaskedMember maskedMember : maskedMembers) {
            response.append("User: `")
                    .append(maskedMember.username.replace("_", "\\_"))
                    .append("` `<@")
                    .append(maskedMember.user_id)
                    .append(">` ");
            DBNation nation = maskedMember.nation_id == null ? null
                    : Locutus.imp().getNationDB().getNationById(maskedMember.nation_id);
            if (nation != null) {
                ...
                response.append(nation.getName())
                        .append(" | <")
                        .append(nation.getUrl())
                        .append("> | ")
                        .append(active)
                        .append(" | ")
                        .append(Rank.byId(nation.getPosition()))
                        .append(" in AA:")
                        .append(nation.getAllianceName());
            }
            response.append("- ").append(maskedMember.reason).append("\n");
        }
        return response.toString();
    }
    ...
    @NoFormat
    @RolePermission(value = Roles.MEMBER, onlyInGuildAlliance = true)
    public static String AllianceSheet(AlliancePlaceholders aaPlaceholders, @Me @Default Guild guild,
            @Me IMessageIO channel,
            @Me @Default DBNation me,
            @Me @Default User author, @Me @Default GuildDB db,
            @Arg("The nations to include in each alliance") Set<DBNation> nations,
            @Arg("The columns to use in the sheet") @TextArea List<String> columns,
        ...
        // Placeholders.PlaceholderCache<DBNation> natCache = new
        // Placeholders.PlaceholderCache<>(totals.values());
    ...
    @NoFormat
    @RolePermission(value = Roles.MEMBER, onlyInGuildAlliance = true)
    public static void NationSheet(NationPlaceholders placeholders, @Me IMessageIO channel, @Me @Default DBNation me,
            @Me @Default User author, @Me @Default GuildDB db,
            NationList nations,
            @Arg("""
                    A space separated list of columns to use in the sheet
                    Can include NationAttribute as placeholders in columns
                    All NationAttribute placeholders must be surrounded by {} e.g. {nation}""") @TextArea List<String> columns,
        ...
        for (String arg : columns) {
            formatFunction.add(
                    placeholders.getFormatFunction(db == null ? null : db.getGuild(), me, author, arg, cache, true));
        }
    ...
    public String multi(@Me IMessageIO channel, @AllowDeleted DBNation nation) {
        ...
        if (result.length() + title.length() > MessageEmbed.EMBED_MAX_LENGTH_BOT
                || result.length() > MessageEmbed.DESCRIPTION_MAX_LENGTH) {
            String condensed = report.toString(true);
            if (condensed.length() + title.length() <= MessageEmbed.EMBED_MAX_LENGTH_BOT
                    && condensed.length() <= MessageEmbed.DESCRIPTION_MAX_LENGTH) {
        ...
        if (Settings.INSTANCE.ENABLED_COMPONENTS.WEB) {
            disclaimer += ("\n**See also:** " + Settings.INSTANCE.WEB.FRONTEND_DOMAIN + "/#/multi_v2/"
                    + nation.getId());
        }
    ...
    @Command(desc = "Return quickest attacks to beige an enemy at a resistance level", aliases = { "fastBeige",
            "quickestBeige", "quickBeige", "fastestBeige" }, viewable = true)
    public String quickestBeige(@Range(min = 1, max = 100) int resistance,
            @Arg("Don't allow ground attacks") @Switch("g") boolean noGround,
            @Arg("Don't allow naval attacks") @Switch("s") boolean noShip,
            @Arg("Don't allow aircraft attacks") @Switch("a") boolean noAir,
            @Arg("Don't allow missile attacks") @Switch("m") boolean noMissile,
            @Arg("Don't allow nuclear attacks") @Switch("n") boolean noNuke) {
        if (resistance > 1000 || resistance < 1)
            throw new IllegalArgumentException("Resistance must be between 1 and 100");
        List<AttackType> allowed = new ArrayList<>(List.of(AttackType.values));
        if (noGround)
            allowed.removeIf(f -> f == AttackType.GROUND);
        if (noShip)
            allowed.removeIf(f -> f.getUnits().length > 0 && f.getUnits()[0] == MilitaryUnit.SHIP);
        if (noAir)
            allowed.removeIf(f -> f.getUnits().length > 0 && f.getUnits()[0] == MilitaryUnit.AIRCRAFT);
        if (noMissile)
            allowed.removeIf(f -> f.getUnits().length > 0 && f.getUnits()[0] == MilitaryUnit.MISSILE);
        if (noNuke)
            allowed.removeIf(f -> f.getUnits().length > 0 && f.getUnits()[0] == MilitaryUnit.NUKE);
    ...
    @IsAlliance
    public String setBracketBulk(CommandRuntimeLookupContext services, NationDB nationDb, @Me JSONObject command,
            @Me IMessageIO io, @Me GuildDB db, @Me User author, SpreadSheet sheet, @Switch("f") boolean force) {
        ...
        int max = Math
                .max(Math.max(Math.max(nations == null ? 0 : nations.size(), leaders == null ? 0 : leaders.size()),
                        brackets == null ? 0 : brackets.size()), internals == null ? 0 : internals.size());
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (nationStr != null) {
                try {
                    nation = PWBindings.parseNation(services, null, db.getGuild(), nationStr, null);
                } catch (IllegalArgumentException e) {
                    errors.add("[Row:" + (i + 2) + "] Nation not found: `" + nationStr + "`");
                    continue;
                }
            } else {
                nation = PWBindings.parseNation(services, null, db.getGuild(), "leader:" + leaderStr, null);
                if (nation == null) {
            ...
            if (bracketStr != null && !bracketStr.isEmpty()) {
                TaxBracket bracket = bracketsByStr.computeIfAbsent(bracketStr,
                        s -> PWBindings.parseBracket(nationDb, db, s, 60000L, true));
        ...
        if (!force) {
            ...
            Set<Integer> fromBracketIds = setBracket.keySet().stream().map(DBNation::getTax_id)
                    .collect(Collectors.toSet());
            Set<Integer> toBracketIds = setBracket.values().stream().map(TaxBracket::getId).collect(Collectors.toSet());
            Set<TaxRate> fromInternalRates = setInternal.keySet().stream().map(n -> internalRates.get(n.getId()))
                    .filter(Objects::nonNull).collect(Collectors.toSet());
            Set<TaxRate> toInternalRates = new HashSet<>(setInternal.values());
            if (!setBracket.isEmpty()) {
                body.append("Change " + setBracket.size() + " nations from " + fromBracketIds.size()
                        + " unique tax bracket(s) to " + toBracketIds.size() + " bracket(s)\n");
            }
            if (!setInternal.isEmpty()) {
                body.append("Change " + setInternal.size() + " nations from " + fromInternalRates.size()
                        + " unique internal tax rate(s) to " + toInternalRates.size() + " rate(s)\n");
            }
        ...
        for (Map.Entry<DBNation, TaxBracket> entry : setBracket.entrySet()) {
            DBNation nation = entry.getKey();
            TaxBracket bracket = entry.getValue().withLookup(Locutus.imp().getNationDB());
    ...
    public String me(@Me JSONObject command, @Me @Default Guild guild, @Me IMessageIO channel, @Me DBNation me,
            @Me User author, @Me @Default GuildDB db, @Switch("s") @Timestamp Long snapshotDate) throws IOException {
        return who(command, guild, channel, author, db, me, Collections.singleton(me), null, false, false, false, false,
                false, false, snapshotDate, null);
    }

    @Command(aliases = { "who", "pnw-who", "who", "pw-who", "pw-info", "how", "where", "when", "why",
            "whois" }, desc = """
    ...
    @UserCommand
    public static String who(@Me JSONObject command, @Me @Default Guild guild, @Me IMessageIO channel,
            @Me @Default User author, @Me @Default GuildDB db, @Me @Default DBNation me,
            @Arg("The nations to get info about") Set<NationOrAlliance> nationOrAlliances,
            @Arg("Sort any listed nations by this attribute") @NoFormat @Default() TypedFunction<DBNation, Double> sortBy,
            @Arg("List the nations instead of just providing a summary") @Switch("l") boolean list,
            @Arg("List the alliances of the provided nation") @Switch("a") boolean listAlliances,
            @Arg("List the discord user ids of each nation") @Switch("r") boolean listRawUserIds,
            @Arg("List the discord user mentions of each nation") @Switch("m") boolean listMentions,
            @Arg("List paginated info of each nation") @Switch("i") boolean listInfo,
            @Arg("List all interview channels of each nation") @Switch("c") boolean listChannels,
        ...
        if (filter == null && me != null)
            filter = me.getQualifiedId();
        final Set<DBNation> nations = PW.getNationsSnapshot(SimpleNationList.from(nationOrAlliances).getNations(),
                filter, snapshotDate, db == null ? null : db.getGuild());
        ...
        if ((snapshotDate == null && nationOrAlliances.size() == 1) || nations.size() == 1) {
            ...
            if (nationOrAA.isNation()) {
                ...
                if ((myNation != null && myNation.getId() == nation.getId())
                        || (guild != null && Roles.INTERNAL_AFFAIRS_STAFF.has(author, guild))) {
                ...
                // Departures
                CM.nation.departures departures = CM.nation.departures.cmd.nationOrAlliance(nation.getUrl())
                        .time("9999d");
                ...
                CM.nation.score score = CM.nation.score.cmd.nation(nation.getUrl()).missiles("").nukes("").projects("")
                        .avg_infra("").builtMMR("");
                ...
                if (db != null) {
                    ...
                    CM.war.counter.nation counter = CM.war.counter.nation.cmd.target(nation.getUrl())
                            .counterWith(aaIdStr).ping("");
                ...
                CM.alliance.stats.metricsByTurn militarization = CM.alliance.stats.metricsByTurn.cmd
                        .metric(AllianceMetric.GROUND_PCT.name()).coalition(alliance.getQualifiedId()).start("7d");
                msg = msg.commandButton(CommandBehavior.EPHEMERAL, militarization, "Military Graph");
                // Tiering graph
                CM.stats_tier.cityTierGraph tiering = CM.stats_tier.cityTierGraph.cmd
                        .coalition1(alliance.getQualifiedId()).coalition2("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, tiering, "City Tier Graph");
                // strength graph
                CM.stats_tier.strengthTierGraph strength = CM.stats_tier.strengthTierGraph.cmd
                        .coalition1(alliance.getQualifiedId()).coalition2("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, strength, "Strength Tier Graph");
                // mmr tier
                CM.stats_tier.mmrTierGraph mmr = CM.stats_tier.mmrTierGraph.cmd.coalition1(alliance.getQualifiedId())
                        .coalition2("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, mmr, "MMR Tier Graph");
                // spy tier
                CM.stats_tier.spyTierGraph spy = CM.stats_tier.spyTierGraph.cmd.coalition1(alliance.getQualifiedId())
                        .coalition2("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, spy, "Spy Tier Graph");

                // - /coalition create - add
                CM.coalition.create createCoalition = CM.coalition.create.cmd.alliances(alliance.getQualifiedId())
                        .coalitionName("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, createCoalition, "Create Coalition");

                // - /coalition generate - sphere
                CM.coalition.generate generateCoalition = CM.coalition.generate.cmd.coalition("")
                        .rootAlliance(alliance.getQualifiedId()).topX("80");
                msg = msg.modal(CommandBehavior.EPHEMERAL, generateCoalition, "Add Sphere Coalition");
                // - /alliance departures
                CM.alliance.departures departures = CM.alliance.departures.cmd
                        .nationOrAlliance(alliance.getQualifiedId()).time("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, departures, "Departures");
                // - loot
                CM.nation.loot loot = CM.nation.loot.cmd.nationOrAlliance(alliance.getQualifiedId()).nationScore("");
                msg = msg.modal(CommandBehavior.EPHEMERAL, loot, "Loot");

                // alliance cost
                CM.alliance.cost cost = CM.alliance.cost.cmd.nations(alliance.getQualifiedId());
                msg = msg.commandButton(CommandBehavior.EPHEMERAL, cost, "Cost");
                // offshore find
                CM.offshore.find.for_coalition findOffshore = CM.offshore.find.for_coalition.cmd
            ...
            {
                Set<String> toRemoveLower = Set.of("list", "listalliance", "listrawuserids", "listmentions", "listinfo",
                        "listchannels", "page");
        ...
        if (listAny) {
            ...
            if (listAlliances) {
                // alliances
                List<DBAlliance> alliancesSorted = nations.stream().map(DBNation::getAlliance).filter(Objects::nonNull)
        ...
        if (listInfo) {
            ...
            if (sortBy != null) {
                Collections.sort(sorted, (o1, o2) -> Double.compare(((Number) sortBy.apply(o2)).doubleValue(),
                        ((Number) sortBy.apply(o1)).doubleValue()));
            }
            ...
            for (DBNation nation : sorted) {
                ...
                // Activity activity = nation.getActivity(14 * 12);
                // double loginChance = activity.loginChance((int) Math.max(1, (12 -
                // (currentTurn % 12))), true);
    ...
    public String interest(ValueStore store, @Me IMessageIO channel, @Me GuildDB db, Set<DBNation> nations,
            @Arg("A percent (out of 100) to apply to POSITIVE resources counts in their account balance") @Range(min = -100, max = 100) double interestPositivePercent,
            @Arg("A percent (out of 100) to apply to NEGATIVE resources counts in their account balance") @Range(min = -100, max = 100) double interestNegativePercent)
            throws IOException, GeneralSecurityException {
        if (nations.isEmpty())
            throw new IllegalArgumentException("No nations specified");
        if (nations.size() > 180)
            throw new IllegalArgumentException("Cannot do intest to that many people");
    ...
    @Command(aliases = { "dnr",
            "caniraid" }, desc = "Check if declaring war on a nation is allowed by the guild's Do Not Raid (DNR) settings", viewable = true)
    public String dnr(@Me GuildDB db, DBNation nation) {
        ...
        if (!dnr) {
            ...
            title = ("You CAN raid " + nation.getNation()
                    + " (however they are an active member of an alliance), see also: "
                    + CM.alliance.stats.counterStats.cmd.toSlashMention());
        } else if (nation.getPosition() > 1) {
            title = "You CAN raid " + nation.getNation() + " (however they are a member of an alliance), see also: "
                    + CM.alliance.stats.counterStats.cmd.toSlashMention();
        } else if (nation.getAlliance_id() != 0) {
            title = "You CAN raid " + nation.getNation() + ", see also: "
                    + CM.alliance.stats.counterStats.cmd.toSlashMention();
        } else {
            title = "You CAN raid " + nation.getNation();
        }
        ...
        if (dnrTopX != null) {
            ...
            response.append("\n> - Avoid members of the top " + dnrTopX
                    + " alliances and members of direct allies (check ingame treaties)");

            Set<String> enemiesStr = enemies.stream().map(f -> Locutus.imp().getNationDB().getAllianceName(f))
                    .filter(Objects::nonNull).collect(Collectors.toSet());
            Set<String> canRaidStr = canRaid.stream().map(f -> Locutus.imp().getNationDB().getAllianceName(f))
                    .filter(Objects::nonNull).collect(Collectors.toSet());
            Set<String> canRaidInactiveStr = canRaidInactive.stream()
                    .map(f -> Locutus.imp().getNationDB().getAllianceName(f)).filter(Objects::nonNull)
                    .collect(Collectors.toSet());
    ...
    public String setLoot(DBNation nation, Map<ResourceType, Double> resources,
            @Default("ESPIONAGE") NationLootType type, @Default("1") double fraction) {
        ...
        Locutus.imp().runEventsAsync(
                events -> LootEntry.forNation(nation.getNation_id(), System.currentTimeMillis(), rssArr, type)
                        .save(events));
        return "Set " + nation.getNation() + " to " + ResourceType.toString(resources) + " worth: ~$"
                + ResourceType.convertedTotal(resources);
    }
    ...
    private void checkAndUpdateStreak(Map<Integer, Long> lastActive, Map<Integer, Long> lastNotGray,
            Map<Integer, Long> lastStreak, Map<Integer, Integer> countMap, int nationId, long day, int daysInactive) {
        ...
        if (lastActiveAdded == lastActiveDay)
            return;
    ...
    public String grayStreak(@Me @Default GuildDB db, @Me IMessageIO io,
        ...
        Set<Integer> nationIds = new IntOpenHashSet(
                nations.stream().map(DBNation::getNation_id).collect(Collectors.toSet()));
        ...
        parser.iterateAll(f -> f >= finalMinDay,
                (h, r) -> r.required(h.nation_id).optional(h.vm_turns).required(h.color), null,
                new BiConsumer<Long, NationHeaderReader>() {
                    @Override
                    public void accept(Long day, NationHeaderReader r) {
                        ...
                        if (color != NationColor.GRAY) {
                            ...
                            checkAndUpdateStreak(lastActive, lastNotGray, lastStreak, countMap, nationId, day,
                                    daysInactive);
                        }
                    }
    ...
    public static String vmHistory(@Me IMessageIO io, @Me @Default GuildDB db, Set<DBNation> nations,
        ...
        CompletableFuture<IMessageBuilder> msgFuture = io.sendIfFree("Mounting nation snapshots...");
        ...
        if (sheet != null) {
            ...
            IMessageBuilder msg = io.create();
            if (!hasData)
                msg.append("No VM history found");
        ...
        // }
        //
        // Map<Integer, DBNation> nationMap =
        // nations.stream().collect(Collectors.toMap(DBNation::getNation_id, f -> f));
        ...
        // contains = f -> f == natId;
        // } else {
        // Set<Integer> natIds = new
        // IntOpenHashSet(nations.stream().map(DBNation::getNation_id).collect(Collectors.toSet()));
        ...
        // parser.iterateAll(f -> f >= finalMinDay, (h, r) ->
        // r.required(h.nation_id).optional(h.vm_turns), null, new BiConsumer<Long,
        // NationHeader>() {
        ...
        // daysVM.computeIfAbsent(nationId, f -> new
        // ObjectLinkedOpenHashSet<>()).add(day);
        // } else {
        // daysPresentNotVM.computeIfAbsent(nationId, f -> new
        // ObjectLinkedOpenHashSet<>()).add(day);
        // }
        // }
        // }, null, new Consumer<Long>() {
        ...
        // lastMsg[0] = now;
        // io.updateOptionally(msgFuture, (day - finalMinDay) + "/" + (today -
        // finalMinDay));
        // }
        // }
        // });
        ...
        // Set<Long> vmDays = daysVM.getOrDefault(nation.getNation_id(),
        // Collections.emptySet());
        // Set<Long> presentNotVMDays =
        // daysPresentNotVM.getOrDefault(nation.getNation_id(), Collections.emptySet());
        //
        // long currentStatus;
        ...
        // changesByNation.computeIfAbsent(nation.getNation_id(), k -> new
        // ArrayList<>())
        // .add(new String[]{TimeUtil.format(TimeUtil.DD_MM_YYYY, timeMs),
        // statusString});
        // }
        ...
        //
        // if (sheet != null) {
        // List<String> header = new ArrayList<>(Arrays.asList("nation", "date",
        // "status"));
        // sheet.setHeader(header);
    ...
    @Command(aliases = { "alliancecost",
            "aacost" }, desc = "Get the value of nations including their cities, projects and units", viewable = true)
    public String allianceCost(ValueStore store, @Me IMessageIO channel, @Me @Default GuildDB db,
            ...
            @Switch("s") @Timestamp Long snapshotDate) {
        Set<DBNation> nationSet = PW.getNationsSnapshot(nations.getNations(), nations.getFilter(), snapshotDate,
                db == null ? null : db.getGuild());
        ...
        for (DBNation nation : nationSet) {
            ...
            Set<Project> projects = nation.getProjects();
            for (Project project : projects) {
                if (project == Projects.URBAN_PLANNING)
                    continue;
                if (project == Projects.ADVANCED_URBAN_PLANNING)
                    continue;
                if (project == Projects.METROPOLITAN_PLANNING)
                    continue;

                if (includeProjects != null && !includeProjects.contains(project))
                    continue;
                projectCost = ResourceType.addResourcesToA(projectCost, project.cost());
            }
            for (MilitaryUnit unit : MilitaryUnit.values) {
                int units = nation.getUnits(unit);
                militaryCost = ResourceType.addResourcesToA(militaryCost,
                        unit.getCost(units, f -> nation.getResearch(cacheStore, f)));
            }
            int cities = nation.getCities();
            double nationCityCost = 0;
            for (int i = 1; i <= cities; i++) {
                boolean manifest = true;
                // boolean cp = i > Projects.URBAN_PLANNING.requiredCities() &&
                // projects.contains(Projects.URBAN_PLANNING);
                // boolean acp = i > Projects.ADVANCED_URBAN_PLANNING.requiredCities() &&
                // projects.contains(Projects.ADVANCED_URBAN_PLANNING);
                // boolean mp = i > Projects.METROPOLITAN_PLANNING.requiredCities() &&
                // projects.contains(Projects.METROPOLITAN_PLANNING);
            ...
            for (Map.Entry<Integer, JavaCity> cityEntry : cityMap.entrySet()) {
                JavaCity city = cityEntry.getValue();
                {
                    ...
                    if (projects.contains(Projects.ARABLE_LAND_AGENCY))
                        landFactor *= 0.95;
                    if (projects.contains(Projects.CENTER_FOR_CIVIL_ENGINEERING))
                        infraFactor *= 0.95;
                    landCost += PW.City.Land.calculateLand(PW.City.Land.NEW_CITY_BASE, city.getLand()) * landFactor;
                    infraCost += PW.City.Infra.calculateInfra(PW.City.Infra.NEW_CITY_BASE, city.getInfra())
                            * infraFactor;
                }
    ...
    public String addWatermark(@Me IMessageIO io, String imageUrl, String watermarkText, @Default Color color,
            @Default("0.05") @Range(min = 0.01, max = 1) double opacity, @Default("Arial") Font font,
            @Switch("r") boolean repeat) {
    ...
    public String projectROI(@Me IMessageIO channel, @Me @Default GuildDB db,
        ...
        double originRevenue;
        {
            ...
            JavaCity optimal = new JavaCity(first).zeroNonMilitary().setOptimalPower(nationCopy.getContinent())
                    .optimalBuild(nationCopy.getContinent(), nationCopy.getCities(), INationCity::getRevenueConverted,
                            null, finalHasProject, 15000, nation.getRads(), false, true, nation.getGrossModifier(),
                            null);
            originRevenue = PW.City.profitConverted(nationCopy.getContinent(), nationCopy.getRads(), finalHasProject,
                    nation.getCities(), nation.getGrossModifier(), optimal);
        }

        StringBuilder result = new StringBuilder();
        CompletableFuture<IMessageBuilder> msgFuture = channel
                .send("Please wait. This will take 5s to initialize and then 5s for each project to calculate ROI...");
        ...
        for (int i = 0; i < projectsList.size(); i++) {
            ...
            if (roiFunc == null)
                continue;
            if ((!projectsSpecified || projectsList.size() != 1)
    ...
    // originNation = me;
    // }
    // Continent continentFinal = continent != null ? continent :
    // originNation.getContinent();
    ...
    // Predicate<Building> militaryOrPower = f -> f.getType() ==
    // BuildingType.MILITARY || f.getType() == BuildingType.POWER;
    ...
    // throw new IllegalArgumentException("No spare improvements slots with build of
    // MMR: " + origin.getMMR() + " and infra: " +
    // MathMan.format(infraRequiredFinal));
    // }
    //
    // Predicate<Project> hasProject = forceProjects != null ? f ->
    // forceProjects.contains(f) : Predicates.alwaysFalse();
    // if (originNation != null) hasProject =
    // hasProject.or(originNation::hasProject);
    // hasProject = Projects.optimize(hasProject);
    //
    // double grossModifier = DBNation.getGrossModifier(false,
    // openMarkets || (originNation != null && originNation.getDomesticPolicy() ==
    ...
    //
    // double profit = PW.City.profitConverted(continentFinal, rads, hasProject,
    // 100, grossModifier, copy);
    // if (profit > bestValue) {
    // bestValue = profit;
    // best = copy;
    // }
    ...
    //
    // if (best == null) {
    // return "No build found matching: `" + MathMan.format(infraRequiredFinal) + "`
    // infra, `" + origin.getMMR() + "` MMR";
    // }
    // String title = "~$" + MathMan.format(bestValue) + " profit";
    // StringBuilder body = new StringBuilder();
    // body.append("```\n" + best.toJson(true) + "\n```");
    // body.append("\nDisease:
    // ").append(MathMan.format(best.calcDisease(hasProject)));
    // body.append("\nPollution:
    // ").append(MathMan.format(best.calcPollution(hasProject)));
    // body.append("\nCrime: ").append(MathMan.format(best.calcCrime(hasProject)));
    // body.append("\nCommerce:
    // ").append(MathMan.format(best.calcCommerce(hasProject)) + "/" +
    // MathMan.format(best.getMaxCommerce(hasProject)));
    // body.append("\nPopulation:
    // ").append(MathMan.format(best.calcPopulation(hasProject)));
    //
    ...
    public String infraROI(DBCity city, @Range(min = 600, max = 3500) int infraLevel,
        ...
        if (nation == null)
            return "Unknown nation: `" + city.getNation_id() + "`";
        if (infraLevel > 3500)
            return "Infra level too high (max 3,500)";
        ...
        Predicate<Project> hasProject = forceProjects != null ? f -> forceProjects.contains(f) || nation.hasProject(f)
                : nation::hasProject;
        double grossModifier = DBNation.getGrossModifier(false, openMarkets,
                hasProject.test(Projects.GOVERNMENT_SUPPORT_AGENCY),
                hasProject.test(Projects.BUREAU_OF_DOMESTIC_AFFAIRS));
        ...
        if (profit <= 0)
            return "No ROI for `" + infraLevel + "`";
        double cost = nation.infraCost(infraLevel - 50, infraLevel);
        double daysROI = cost / profit;
        return "Infra: " + MathMan.format(infraLevel - 50) + "->" + MathMan.format(infraLevel) + " will break even in "
                + MathMan.format(daysROI) + " days";
    }
    ...
    public String landROI(DBCity city, @Range(min = 600, max = 10000) double landLevel,
        ...
        if (nation == null)
            return "Unknown nation: `" + city.getNation_id() + "`";
        if (landLevel > 10000)
            return "Land level too high (max 10,000)";
        double rads = rad_index == null ? nation.getRads() : rad_index / 1000d;
        ...
        double profit = revenue1 - revenue2;
        if (profit <= 0)
            return "No ROI for `" + MathMan.format(landLevel) + "`";
        double cost = nation.landCost(landLevel - 50, landLevel);
        double daysROI = cost / profit;
        return "Land: " + MathMan.format(landLevel - 50) + "->" + MathMan.format(landLevel) + " will break even in "
                + MathMan.format(daysROI) + " days";
    }
}