...
public abstract class Placeholders<T, M> extends BindingHelper {
    ...
    public Placeholders(Class<T> type, Class<M> modifierType, ValueStore store, ValidatorStore validators,
    ...
    protected static <T, M> String _addSelectionAlias(Placeholders<T, M> instance, @Me JSONObject command,
            @Me GuildDB db, String name, Set<T> elems, String argumentName) {
        return _addSelectionAlias(instance, "", command, db, name, argumentName);
    }

    protected static <T, M> String _addSelectionAlias(Placeholders<T, M> instance, String prefix,
            @Me JSONObject command, @Me GuildDB db, String name, String... argumentNames) {
    ...
    public SheetTemplate getOrCreateTemplate(GuildDB db, List<String> columns, boolean save,
            AtomicBoolean createdFlag) {
        outer: for (Map.Entry<String, SheetTemplate> entry : db.getSheetManager().getSheetTemplates(getType())
                .entrySet()) {
    ...
    public SelectionAlias getOrCreateSelection(GuildDB db, String selection, String modifier, boolean save,
            AtomicBoolean createdFlag) {
        outer: for (Map.Entry<String, SelectionAlias<T>> entry : db.getSheetManager().getSelectionAliases(getType())
                .entrySet()) {
    ...
    protected static <T, M> String _addColumns(Placeholders<T, M> placeholders, @Me JSONObject command, @Me GuildDB db,
        ...
        if (sheet == null) {
            ...
            throw new IllegalArgumentException(
                    "Sheet type mismatch: `" + sheet.type.getSimpleName() + "` != `" + placeholders.getType() + "`");
        }
    ...
    private void registerCustom(ValueStore store, Method method, Type type) {
        ...
        if (existing != null) {
            if (existing instanceof MethodParser<?> mp) {
                Logg.text("Duplicate placeholder name: " + mp.getMethod().getDeclaringClass().getSimpleName() + " | "
    ...
    public void register(ValueStore store) {
        try {
            ...
            registerCustom(store, methodPredicate,
                    TypeToken.getParameterized(Predicate.class, this.instanceType).getType());
            registerCustom(store, methodFormat,
                    TypeToken.getParameterized(TypedFunction.class, this.instanceType, String.class).getType());
            registerCustom(store, methodDouble,
                    TypeToken.getParameterized(TypedFunction.class, this.instanceType, Double.class).getType());
            registerCustom(store, methodFormatSet, TypeToken
                    .getParameterized(Set.class,
                            TypeToken.getParameterized(TypedFunction.class, this.instanceType, String.class).getType())
                    .getType());
            registerCustom(store, methodDoubleSet, TypeToken
                    .getParameterized(Set.class,
                            TypeToken.getParameterized(TypedFunction.class, this.instanceType, Double.class).getType())
                    .getType());
    ...
    public List<CommandCallable> getFilterCallables() {
        ...
        result.removeIf(cmd -> {
            ...
            return type != String.class && type != boolean.class && type != Boolean.class && type != int.class
                    && type != Integer.class && type != double.class && type != Double.class && type != long.class
                    && type != Long.class && !Enum.class.isAssignableFrom(ReflectionUtil.getClassType(type));
        });
    ...
    public String getDSL(PWGPTHandler gpt, ValueStore store, String query, int numFilters, DSLType type) {
        ...
        if (type == DSLType.FORMATTER_STRING || type == DSLType.FORMATTER_NUMBER
                || type == DSLType.FORMATTER_NUMBER_ARRAY || type == DSLType.FORMATTER_STRING_ARRAY) {
            ...
            String argDesc = ((type == DSLType.FORMATTER_NUMBER_ARRAY || type == DSLType.FORMATTER_STRING_ARRAY)
                    ? "DSL: Array of formatters "
                    : "DSL: formatter ") +
                    """
                    ...
                    if (params.isEmpty()) {
                        ...
                        sig = filter.getPrimaryCommandId() + "(" +
                                params.entrySet().stream()
                                        .map(e -> e.getKey() + ":"
                                                + e.getValue().getBinding().getKey().toSimpleString())
        ...
        // TODO FIXME CM REF add mention of using tool call for listing more filters and
        // for getting more detailed filter info

    }

    // public Map<String, Object> toPredicateJson(ISourceManager embedSrc, String
    // query) {
    ...
    //
    // }
    // //- A Google Sheet URL containing a column of alliance identifiers (one of:
    // alliance, {id}, {name}, {getname}, {getid})
    // //
    // //Filters:
    ...
    protected String wrapHashLegacy(ValueStore store2, String input) {
        if (input.contains("%")) {
            if (input.contains("%guild_alliances%")) {
                Supplier<GuildDB> dbLazy = ArrayUtil
                        .memorize(() -> (GuildDB) store2.getProvided(Key.of(GuildDB.class, Me.class), false));
                GuildDB db = dbLazy.get();
                if (db != null) {
                    String value = db.getAllianceIds(true).stream().map(f -> "AA:" + f)
                            .collect(Collectors.joining(","));
    ...
    protected Predicate<T> getSingleFilter(ValueStore store, String input) {
        ...
        Predicate<T> adapter;
        if (type == String.class) {
            throw new IllegalArgumentException("Cannot use String as filter: `" + input
                    + "`. Please provide a condition e.g. `" + input + "=blah`");
        } else if (type == boolean.class || type == Boolean.class) {
            adapter = f -> {
                if (f == null)
                    return false;
            ...
            throw new IllegalArgumentException(
                    "Only the following filter types are supported: String, Number, Boolean, not: `"
    ...
    public TypedFunction<T, ?> formatRecursively(ValueStore store, String input, ParameterData param, int depth,
            boolean skipFormat, boolean throwError) {
        input = input.trim();
        int currentIndex = 0;
            ...
            while (currentIndex < input.length()) {
                ...
                if (currentChar != '{') {
                    ...
                    if (closingBraceIndex != -1) {
                        ...
                        if (throwError) {
                            throw new IllegalArgumentException(
                                    "Invalid input: Missing closing curly brace: `" + input + "`");
                        }
                        currentIndex++;
                    }
                }
            ...
            try {
                if ((hasPlaceholder || (hasCurlyBracket && param != null)) && !hasNonPlaceholder) {
                    Function<String, Function<T, Object>> stringToParser = s -> {
                        ...
                        return ResolvedFunction.createConstant(parsed != null ? parsed.getClass() : Object.class,
                                parsed, s);
                    };
                    List<LazyMathEntity<T>> lazies = ArrayUtil.calculate(input,
                            s -> new LazyMathEntity<>(s, stringToParser, false));
                    if (lazies.size() == 1) {
        ...
        boolean isResolved = functions.isEmpty()
                || functions.values().stream().allMatch(ResolvedFunction.class::isInstance);
        ...
        if (s instanceof String str) {
            ...
            if (str.startsWith("{") && str.endsWith("}")) {
                if (param != null) {
                    ...
                    if (parsed == null) {
                        throw new IllegalArgumentException(
                                "Null parsed `" + mathFunc.getKey() + "` for Type->Math Expression");
                    }
                    if (parsed instanceof ArrayUtil.DoubleArray da) {
                        return da;
                    }
                    throw new IllegalArgumentException("Cannot parse `" + mathFunc.getKey() + "` invalid type: "
                            + parsed.getClass() + " (expected DoubleArray)");
                }
                if (throwForUnknown) {
                    throw new IllegalArgumentException(
                            "Unknown numeric `" + str + "` cannot parse to DoubleArray for " + param);
                }
            }
            return str;
    ...
    private Map<String, String> explodeArguments(ValueStore store, ParametricCallable command, String argumentString,
        ...
        for (String s : split) {
            ...
            if (indexPar != -1 && (indexSpace == -1 || indexPar < indexSpace)) {
                if (!arg.endsWith(")")) {
                    if (throwError) {
                        throw new IllegalArgumentException(
                                "Invalid input, Missing closing parenthesis for: `" + arg + "`");
                    }
                    return null;
                }
            ...
            if (previousFunc != null) {
                ...
                if (placeholders == null) {
                    throw new IllegalArgumentException("Cannot call `" + arg + "` on function: `"
                            + previousFunc.getName() + "` as return type is not public: `"
                            + ((Class<?>) previousFunc.getType()).getSimpleName() + "`");
                }
            }
            ...
            if (!argumentString.isEmpty()) {
                ...
                for (Map.Entry<String, ParameterData> entry : command.getUserParameterMap().entrySet()) {
                    ...
                    if (explodedArguments.containsKey(argumentName)) {
                        TypedFunction<T, ?> argumentValue = formatRecursively(store,
                                explodedArguments.get(argumentName), param, depth + 1, false, throwError);
            ...
            if (previousFunc == null) {
                ...
                previousFunc = ResolvedFunction.createConstant(function.getType(), function.applyCached(null),
                        functionContent);
            } else if (previousFunc.isResolved()) {
                T value = (T) previousFunc.applyCached(null);
                previousFunc = ResolvedFunction.createConstant(function.getType(), function.applyCached(value),
                        functionContent);
            } else {
                previousFunc = TypedFunction.createParent(function.getType(), previousFunc.andThen((Function) function),
                        functionContent, previousFunc);
            }
        }
        return previousFunc;
    }

    private TypedFunction<T, ?> handleParameterized(ValueStore store, String functionName, String argumentString,
            TypedFunction<T, ?> previousFunc, int depth, boolean throwError) {
        ...
        if (!rawClass.isAssignableFrom(Map.class))
            return null;
        Type[] args = pt.getActualTypeArguments();
        Type keyType = args[0];
        Type valueType = args[1];
        if (!(keyType instanceof Class keyClass && keyClass.isEnum()))
            return null;
        // get enum options
    ...
    private TypedFunction<T, ?> format(ValueStore store, ParametricCallable command,
            Map<String, TypedFunction<T, ?>> arguments) {
        ...
        BiFunction<T, Object[], Object> format = (object, paramVals) -> object == null ? null
                : command.call(object, store, paramVals);
        if (isResolved) {
            Object[] argArr = resolved.apply(null);
            return TypedFunction.createParents(command.getReturnType(), f -> format.apply(f, argArr),
                    "{" + full.toString() + "}", null);
        }
        return TypedFunction.createParents(command.getReturnType(), f -> format.apply(f, resolved.apply(f)),
                "{" + full.toString() + "}", null);
    }
    ...
    public LocalValueStore createLocals(Guild guild, User user, DBNation nation, PlaceholderCache cache) {
        ...
        if (cache != null) {
            PlaceholderCache.addScope(locals, getType(), cache);
        }
        return locals;
    }
    ...
    public Function<T, String> getFormatFunction(Guild callerGuild, DBNation callerNation, User callerUser, String arg,
            Set<T> elems) {
    ...
    public Function<T, String> getFormatFunction(Guild callerGuild, DBNation callerNation, User callerUser, String arg,
            PlaceholderCache cache, boolean throwError) {
    ...
    // Key key = Key.of(TypeToken.getParameterized(TypedFunction.class, getType(),
    // String.class).getType(), HtmlInput.class);
    // addBinding(store -> {
    // store.addParser(key, new FunctionProviderParser<>(key, (Function<ValueStore,
    // String>) valueStore -> {
    // Guild guild = (Guild) valueStore.getProvided(Key.of(Guild.class, Me.class));
    // ParameterData param = (ParameterData)
    // valueStore.getProvided(ParameterData.class);
    // List<Member> options = new ArrayList<>(guild.getMembers());
    //
    // return multipleSelect(param, options, t -> new
    // KeyValue<>(t.getEffectiveName(), t.getAsMention()), true);
    // }));
    // });
    // List<Coalition> options = Arrays.asList(Coalition.values());
    // return WebUtil.generateSearchableDropdown(param, options, (obj, names,
    // values, subtext) -> {
    // names.add(obj.name());
    ...
    public TypedFunction<T, Double> getDoubleFunction(ValueStore store, String arg) {
        ...
        if (type == boolean.class || type == Boolean.class) {
            ...
            throw new IllegalArgumentException("Only the following filter types are supported: Number, Boolean, not: `"
                    + ((Class<?>) type).getSimpleName() + "` | input: `" + result.getName() + "` (" + arg + ")");
        }
    }
    ...
    public TypedFunction<T, String> getFormatFunction(ValueStore store, String arg, PlaceholderCache cache,
            boolean throwError) {
        ...
        if (cache != null)
            PlaceholderCache.addScope(store, getType(), cache);
    ...
    // }
    //
    // public Map.Entry<Type, Function<T, Object>> getPlaceholderFunction(ValueStore
    // store, String input) {
    // List<String> args;
    // int argStart = input.indexOf('(');
