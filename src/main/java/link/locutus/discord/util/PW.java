...
public final class PW {
    public static class API {
        ...
        private static final long ERROR_STALE_RESET_MS = TimeUnit.MINUTES.toMillis(5); // if no 500s for this long,
                                                                                       // reset counters
        ...
        // Call this when catching exceptions from the primary. Returns true if it looks
        // like a 500.
    ...
    public static final class City {
        ...
        public static class Land {
            ...
            public static double calculateLand(double from, double to) {
                ...
                if (to <= from)
                    return (from - to) * -50;
                if (to > 20000)
                    throw new IllegalArgumentException("Land cannot exceed 10,000");
            ...
            public static double calculateLand(double from, double to, boolean ra, boolean aec, boolean ala,
                    boolean gsa, boolean bda) {
                double factor = 1;
                if (aec)
                    factor -= 0.05;
                if (ala)
        ...
        public static class Infra {
            ...
            private static int getInfraCostCents(double infra) {
                ...
                return (int) Math.round(100
                        * (300d + (Math.pow(Math.max(infra - 10d, 20), (2.2d))) * 0.00140845070422535211267605633803));
            }
            ...
            private static int getInfraCostCents(int infra_cents) {
                ...
                return (int) Math.round(100 * (300d + (Math.pow(Math.max(infra_cents - 1000, 2000) * 0.01, (2.2d)))
                        * 0.00140845070422535211267605633803));
            }

            public static double calculateInfra(double from, double to, boolean aec, boolean cfce, boolean urbanization,
                    boolean gsa, boolean bda) {
                double factor = 1;
                if (aec)
                    factor -= 0.05;
            ...
            public static double calculateInfra(double from, double to) {
                ...
                if (to <= from)
                    return (from - to) * -150;
                if (to > 20000)
                    throw new IllegalArgumentException("Infra cannot exceed 10,000 (" + to + ")");
                long total_cents = 0;
        ...
        public static int getPollution(Predicate<Project> hasProject,
        ...
        public static int getCommerce(Predicate<Project> hasProject,
                Function<link.locutus.discord.apiv1.enums.city.building.Building, Integer> getBuildings,
                int maxCommerce, int commerce) {
        ...
        public static double getCrime(Predicate<Project> hasProject,
                Function<link.locutus.discord.apiv1.enums.city.building.Building, Integer> getBuildings,
                long infra_cents, int commerce) {
        ...
        public static double profitConverted(Continent continent, double rads, Predicate<Project> hasProject,
                int numCities, double grossModifier, ICity city) {
            double profit = 0;
            ...
            double income = Math.max(0,
                    (((commerce * 0.02) * 0.725) + 0.725) * city.calcPopulation(hasProject) * newPlayerBonus)
                    * grossModifier;

            profit += income;

            double basePopulation = city.getInfra() * 100;
            double food = (Math.pow(basePopulation, 2)) / 125_000_000
                    + ((basePopulation) * (1 + Math.log(city.getAgeDays()) / 15d) - basePopulation) / 850;
        ...
        public static double[] profit(Continent continent,
                ...
                int turns,
                ICity city) {
            if (profitBuffer == null)
                profitBuffer = new double[ResourceType.values.length];
            ...
            if (forceUnpowered) {
                ...
                if (setPowered != null)
                    powered = setPowered;
                if (powered && city.getPoweredInfra() < city.getInfra())
                    powered = false;
            }
            ...
            for (link.locutus.discord.apiv1.enums.city.building.Building building : Buildings.values()) {
                ...
                if (amt == 0)
                    continue;
                if (!powered) {
                    if (building instanceof CommerceBuilding || building instanceof MilitaryBuilding
                            || (building instanceof ResourceBuilding
                                    && ((AResourceBuilding) building).getResourceProduced().isManufactured())) {
                        continue;
                    }
                }
            ...
            double income = (((commerce * 0.02d) * 0.725d) + 0.725d) * city.calcPopulation(hasProject) * newPlayerBonus
                    * grossModifier;
        ...
        public static double cityCost(int from, int to, boolean manifestDestiny, boolean govSupportAgency,
                boolean bureauOfDomesticAffairs) {
            double total = 0;
        ...
        // public static double nextCityCost(int currentCity, boolean manifestDestiny,
        // boolean cityPlanning, boolean advCityPlanning, boolean metPlanning, boolean
        // govSupportAgency, boolean bureauOfDomesticAffairs) {
        // double cost = 50000*Math.pow(currentCity - 1, 3) + 150000 * (currentCity) +
        // 75000;
        // if (cityPlanning) {
        // cost -= 50000000;
        // }
        ...
        public static double nextCityCost(int currentCity, boolean manifestDestiny, boolean govSupportAgency,
                boolean bureauOfDomesticAffairs) {
            if (CITY_AVERAGE == -1) {
            ...
            double cost = Math.max(Math.pow(currentCity + 1, 2) * 100000,
                    100000 * Math.pow((currentCity + 1) - (top20AverageQuarter), 3)
                            + 150000 * ((currentCity + 1) - (top20AverageQuarter)) + 75000);
            if (manifestDestiny) {
                double factor = 0.05;
                if (govSupportAgency)
                    factor += 0.025;
    ...
    /**
     * Sum the nation transactions (assumes all transactions are valid and should be
     * added)
     * 
     * @param tracked
     ...
     * @return
     */
    public static Map<DepositType, double[]> sumNationTransactions(DBNation nation, GuildDB guildDB, Set<Long> tracked,
            List<Map.Entry<Integer, Transaction2>> transactionsEntries, boolean forceIncludeExpired,
            boolean forceIncludeIgnored, Predicate<Transaction2> filter) {
        Map<DepositType, double[]> result = new EnumMap<>(DepositType.class);
        BiConsumer<Integer, Transaction2> forEach = createSumNationTransactions(nation, guildDB, tracked,
    ...
    public static BiConsumer<Integer, Transaction2> createSumNationTransactions(DBNation nation, GuildDB guildDB,
            Set<Long> tracked, boolean forceIncludeExpired, boolean forceIncludeIgnored, Predicate<Transaction2> filter,
            Map<DepositType, double[]> result) {
        long start = System.currentTimeMillis();

        boolean allowExpiryDefault = (guildDB.getOrNull(GuildKey.RESOURCE_CONVERSION) == Boolean.TRUE)
                || guildDB.getIdLong() == 790253684537688086L;
        long allowExpiryCutoff = 1635910300000L;
        Predicate<Transaction2> allowExpiry;
        if (forceIncludeExpired)
            allowExpiry = Predicates.alwaysFalse();
        else {
            allowExpiry = transaction2 -> allowExpiryDefault || transaction2.tx_datetime > allowExpiryCutoff;
        }
        ...
        if (allowConversionDefault && nation != null) {
            ...
            if (allowConversionDefault) {
                ...
                if (convertDate != null)
                    forceRssConversionAfter = convertDate;
                else {
        ...
        return (sign, record) -> {
            ...
            boolean allowConversion = (finalAllowConversionDefault && record.tx_datetime > allowConversionDefaultCutoff)
                    || (record.tx_id != -1 && isOffshoreSender);
    ...
    public static void processDeposit(Transaction2 record,
        ...
        for (Map.Entry<DepositType, Object> entry2 : notes3.entrySet()) {
            ...
            switch (tag2) {
                ...
                case CASH:
                    if (allowConversion) {
                        applyCashConversion(guildDB, allowArbitraryConversion, notes3, amount, value2, record, date,
                                rates);
    ...
    private static void applyCashConversion(GuildDB guildDB,
            ...
            long date,
            Function<ResourceType, Double> rates) {
        if (value != null && value instanceof Number n && n.doubleValue() == 0d)
            return;

        boolean hasNonCashRss = false;
        for (ResourceType rss : ResourceType.values) {
            if (rss == ResourceType.MONEY || rss == ResourceType.CREDITS)
                continue;
        ...
        if (cashValue == null) {
            ...
            if (cashValue == null) {
                ...
                for (int i = 0; i < amount.length; i++) {
                    ...
                    if (convert != null && !convert.contains((byte) resource.ordinal()))
                        continue;
    ...
    public static Map.Entry<double[], String> createDepositEmbed(GuildDB db,
            NationOrAllianceOrGuildOrTaxid nationOrAllianceOrGuild, Map<DepositType, double[]> categorized,
            Boolean showCategories, double[] escrowed, long escrowExpire, boolean condenseFormat) {
        boolean withdrawIgnoresGrants = GuildKey.MEMBER_CAN_WITHDRAW_IGNORES_GRANTS.getOrNull(db) == Boolean.TRUE;

        boolean hasEscrowed = (escrowed != null && !ResourceType.isZero(escrowed)
                && ResourceType.convertedTotal(escrowed) > 0);
        ...
        List<String> balanceNotes = new ArrayList<>(
                Arrays.asList("#deposit", "#tax", "#loan", "#grant", "#expire", "#decay"));
        ...
        if (showCategories) {
            if (categorized.containsKey(DepositType.DEPOSIT)) {
                response.append("**`#DEPOSIT`** worth $"
                        + MathMan.format(ResourceType.convertedTotal(categorized.get(DepositType.DEPOSIT))));
                response.append("\n```").append(ResourceType.toString(categorized.get(DepositType.DEPOSIT)))
                        .append("```\n");
            }
            if (categorized.containsKey(DepositType.TAX)) {
                response.append("**`#TAX`** worth $"
                        + MathMan.format(ResourceType.convertedTotal(categorized.get(DepositType.TAX))));
                response.append("\n```").append(ResourceType.toString(categorized.get(DepositType.TAX)))
                        .append("```\n");
            ...
            if (categorized.containsKey(DepositType.LOAN)) {
                response.append("**`#LOAN/#GRANT`** worth $"
                        + MathMan.format(ResourceType.convertedTotal(categorized.get(DepositType.LOAN))));
                response.append("\n```").append(ResourceType.toString(categorized.get(DepositType.LOAN)))
                        .append("```\n");
            }
            if (categorized.containsKey(DepositType.GRANT)) {
                response.append("**`#EXPIRE`** worth $"
                        + MathMan.format(ResourceType.convertedTotal(categorized.get(DepositType.GRANT))));
                response.append("\n```").append(ResourceType.toString(categorized.get(DepositType.GRANT)))
                        .append("```\n");
            }
            if (hasEscrowed) {
                response.append("**" + CM.escrow.withdraw.cmd.toSlashMention() + ":** worth: $"
                        + MathMan.format(ResourceType.convertedTotal(escrowed)));
                if (escrowExpire > 0) {
            ...
            if (categorized.size() > 1) {
                response.append("**Balance:** (`" + StringMan.join(balanceNotes, "`|`") + "`) worth: $"
                        + MathMan.format(ResourceType.convertedTotal(balance)) + ")");
            ...
            if (!ResourceType.isZero(nonBalance)) {
                ...
                response.append("```\n" + ResourceType.toString(nonBalance))
    ...
    public static Integer parseAllianceId(String arg) {
        ...
        if (lower.startsWith("aa:"))
            arg = arg.substring(3);
        else if (lower.startsWith("alliance:"))
            arg = arg.substring(9);
        ...
        if (arg.startsWith(Settings.PNW_URL() + "/alliance/id=")
                || arg.startsWith(Settings.PNW_URL() + "//alliance/id=")
                || arg.startsWith(Settings.PNW_URL() + "/alliance/id=")) {
    ...
    static {
        ...
        for (int i = minCents; i <= max * 100; i++) {
            double x = (i * 0.01) - 10d;
            int cost = Math
                    .toIntExact(Math.round(100 * (300d + (Math.pow(x, (2.2d))) * 0.00140845070422535211267605633803)));
    ...
    public static double[] getRevenue(ValueStore store, double[] profitBuffer, int turns, DBNation nation,
            Collection<JavaCity> cities, boolean militaryUpkeep, boolean tradeBonus, boolean bonus, boolean noFood,
            boolean noPower, double treasureBonus) {
        double rads = nation.getRads();
        boolean atWar = nation.getNumWars() > 0;
        long date = -1L;
        return getRevenue(store, profitBuffer, turns, date, nation, cities, militaryUpkeep, tradeBonus, bonus, noFood,
                noPower, rads, atWar, treasureBonus);
    }

    public static double[] getRevenue(ValueStore store, double[] profitBuffer, int turns, long date, DBNation nation,
            Collection<JavaCity> cities, boolean militaryUpkeep, boolean tradeBonus, boolean bonus, boolean noFood,
            boolean noPower, double rads, boolean atWar, double treasureBonus) {
        if (profitBuffer == null)
            profitBuffer = new double[ResourceType.values.length];
        ...
        // Project revenue
        // if (checkRpc && nation.getCities() <= 15 &&
        // nation.hasProject(Projects.ACTIVITY_CENTER)) {
        //// for (ResourceType type : ResourceType.values) {
        //// if (type.isRaw() && type.getBuilding().canBuild(nation.getContinent())) {
        //// // profitBuffer[type.ordinal()] += turns * (Math.min(nation.getCities(),
        // 10));
        //// }
        //// }
        // }
        ...
        for (JavaCity build : cities) {
            ...
            profitBuffer = build.profit(continent, rads, date, nation::hasProject, profitBuffer, numCities,
                    grossModifier, noPower, 12);
        }
    ...
    public static String getTradeUrl(ResourceType type, boolean isBuy) {
        String url = Settings.PNW_URL() + "/index.php?id=90&display=world&resource1=%s&buysell="
                + (isBuy ? "buy" : "sell") + "&ob=price&od=DEF";
    ...
    public static Map<ScoreType, Double> scoreBreakdown(DBNation nation, MMRDouble mmr, Double infra, Integer projects,
            Integer cities, Integer researchBits) {
        Map<ScoreType, Double> result = new EnumMap<>(ScoreType.class);

        if (projects == null)
            projects = nation.getNumProjects();
        if (researchBits == null)
        ...
        for (int i = 0; i < units.length; i++) {
            ...
            if (mmr != null && unit.getBuilding() != null) {
                amt = (int) (mmr.getPercent(unit) * unit.getBuilding().getUnitCap()
                        * unit.getBuilding().cap(Predicates.alwaysFalse()) * cities);
            } else {
    ...
    public static double estimateScore(ValueStore store, DBNation nation, MMRDouble mmr, Double infra, Integer projects,
            Integer cities, Integer researchBits) {
        ...
        if (projects == null)
            projects = nation.getNumProjects();
        if (researchBits == null)
            researchBits = nation.getResearchBits(store);
        if (infra == null) {
            infra = 0d;
            for (DBCity city : nation._getCitiesV3().values()) {
                infra += city.getInfra();
    ...
    public static BiFunction<Double, Double, Double> getXInRange(Collection<DBNation> attackers,
            Function<DBNation, Double> valueFunc) {
        int minScore = Integer.MAX_VALUE;
        int maxScore = 0;
        for (DBNation attacker : attackers) {
            minScore = (int) Math.min(minScore, attacker.getScore() * 0.75);
    ...
    public static Set<Integer> parseAlliances(GuildDB db, String arg) {
        ...
        for (String allianceName : arg.split(",")) {
            ...
            if (!coalition.isEmpty())
                aaIds.addAll(coalition);
            else {
                Integer aaId = PW.parseAllianceId(allianceName);
                if (aaId == null)
                    throw new IllegalArgumentException("Unknown alliance: `" + allianceName + "`");
                aaIds.add(aaId);
            }
        }
        return aaIds;
