...
public class NationUpdateProcessor {
        ...
        Locutus.imp().getExecutor().submit(new Runnable() {
            ...
            public void run() {
                for (Map.Entry<Integer, Integer> entry : membersByAA.entrySet()) {
                    ...
                    AlertUtil.forEachChannel(f -> true, GuildSettings.Key.ACTIVITY_ALERTS, new BiConsumer<MessageChannel, GuildDB>() {
                        @Override
    ...
    public void onNationCreate(NationCreateEvent event) {
        ...
        Locutus.imp().getExecutor().submit(() -> {
            ...
            if (rerollId > 0) {
                ...
                AlertUtil.forEachChannel(f -> true, GuildSettings.Key.REROLL_ALERT_CHANNEL, new BiConsumer<MessageChannel, GuildDB>() {
                    @Override
                    public void accept(MessageChannel channel, GuildDB guildDB) {
                        DiscordUtil.createEmbedCommand(channel, title, body.toString());
                    }
    ...
    private void checkOfficerChange(DBNation previous, DBNation current) {
        ...
        if (alliance != null && alliance.getRank() < 50)
        {
            ...
            AlertUtil.forEachChannel(f -> true, GuildSettings.Key.ORBIS_OFFICER_LEAVE_ALERTS, new BiConsumer<MessageChannel, GuildDB>() {
                @Override
                public void accept(MessageChannel channel, GuildDB guildDB) {
...
//                    leftVMBeige = true;
        ...
        AlertUtil.forEachChannel(GuildDB::isValidAlliance, GuildSettings.Key.ENEMY_ALERT_CHANNEL, new BiConsumer<MessageChannel, GuildDB>() {
            @Override
            public void accept(MessageChannel channel, GuildDB guildDB) {
                EnemyAlertChannelMode mode = guildDB.getOrNull(GuildSettings.Key.ENEMY_ALERT_CHANNEL_MODE);
...
//                                        continue;
    ...
    private boolean raidAlert(DBNation defender) {
        ...
        String finalMsg = msg;
        AlertUtil.forEachChannel(f -> f.hasCoalitionPermsOnRoot(Coalition.RAIDPERMS), GuildSettings.Key.BEIGE_ALERT_CHANNEL, new BiConsumer<MessageChannel, GuildDB>() {
            @Override
            public void accept(MessageChannel channel, GuildDB guildDB) {
    ...
    public void onNationChangeName(NationChangeNameEvent event) {
        ...
        if (user != null) {
            for (Guild guild : user.getMutualGuilds()) {
                ...
                if (db.getOrNull(GuildSettings.Key.AUTONICK) == GuildDB.AutoNickOption.NATION) {
                    try {
    ...
    public void onNationChangeLeader(NationChangeLeaderEvent event) {
        ...
        if (user != null) {
            for (Guild guild : user.getMutualGuilds()) {
                ...
                if (db.getOrNull(GuildSettings.Key.AUTONICK) == GuildDB.AutoNickOption.LEADER) {
                    try {
                        db.getAutoRoleTask().autoRole(member, System.out::println);
                    } catch (Throwable e) {
                        e.printStackTrace();
    ...
    private static void checkExodus(DBNation previous, DBNation current) {
        ...
        if (memberRemoves >= 5) {
            ...
            AlertUtil.forEachChannel(f -> true, GuildSettings.Key.ORBIS_ALLIANCE_EXODUS_ALERTS, new BiConsumer<MessageChannel, GuildDB>() {
                @Override
                public void accept(MessageChannel channel, GuildDB guildDB) {
                    DiscordUtil.createEmbedCommand(channel, title, body);
                }
        ...
        if (total > 10000000) {
            ...
            try {
                for (Map.Entry<Long, GuildDB> entry : Locutus.imp().getGuildDatabases().entrySet()) {
                    ...
                    MessageChannel channel = guildDB.getOrNull(GuildSettings.Key.BANK_ALERT_CHANNEL, false);
                    if (channel == null) {
                        continue;
                    }
    ...
    private static void processLeaderChange(DBNation previous, DBNation current) {
        ...
        if (isRelevant) {
            AlertUtil.forEachChannel(f -> true, GuildSettings.Key.ORBIS_LEADER_CHANGE_ALERT, new BiConsumer<MessageChannel, GuildDB>() {
                @Override
                public void accept(MessageChannel channel, GuildDB guildDB) {
    ...
    public static void processDeletion(DBNation previous, DBNation current) {
        if (previous.getActive_m() < 14400 && current == null) {
            ...
            AlertUtil.forEachChannel(BankAlerts.class, GuildSettings.Key.DELETION_ALERT_CHANNEL, new BiConsumer<MessageChannel, GuildDB>() {
                @Override
