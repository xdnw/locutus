...
public class AutoRoleTask implements IAutoRoleTask {
    ...
    public synchronized void syncDB() {
        GuildDB.AutoNickOption nickOpt = db.getOrNull(GuildSettings.Key.AUTONICK);
        if (nickOpt != null) {
            setNickname(nickOpt);
        }

        GuildDB.AutoRoleOption roleOpt = db.getOrNull(GuildSettings.Key.AUTOROLE);
        if (roleOpt != null) {
            try {
        ...
        this.autoRoleAllyGov = Boolean.TRUE == db.getOrNull(GuildSettings.Key.AUTOROLE_ALLY_GOV);

        this.autoRoleRank = db.getOrNull(GuildSettings.Key.AUTOROLE_ALLIANCE_RANK);
        if (this.autoRoleRank == null) this.autoRoleRank = Rank.MEMBER;
        allowedAAs = null;
        Integer topX = db.getOrNull(GuildSettings.Key.AUTOROLE_TOP_X);
        if (topX != null) {
    ...
    public void autoRoleAllies(Consumer<String> output, Set<Member> members) {
        ...
        if (!members.isEmpty()) {
            ...
            Set<Roles> maskRolesSet = db.getOrNull(GuildSettings.Key.AUTOROLE_ALLY_ROLES);
