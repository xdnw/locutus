...
public class RawsTemplate extends AGrantTemplate<Integer> {
    ...
    public RawsTemplate(GuildDB db, boolean isEnabled, String name, NationFilter nationFilter, long econRole,
        ...
        this(db, isEnabled, name, nationFilter, econRole, selfRole, fromBracket, useReceiverBracket, maxTotal, maxDay,
                maxGranterDay, maxGranterTotal, rs.getLong("date_created"), rs.getLong("days"),
                rs.getLong("overdraw_percent_cents"),
                rs.getLong("expire"),
                rs.getLong("decay"),
    ...
    public RawsTemplate(GuildDB db, boolean isEnabled, String name, NationFilter nationFilter, long econRole,
            long selfRole, int fromBracket, boolean useReceiverBracket, int maxTotal, int maxDay, int maxGranterDay,
            int maxGranterTotal, long dateCreated, long days, long overdrawPercentCents, long expiryOrZero,
            long decayOrZero, boolean allowIgnore, long repeatable_time) {
    ...
    public List<Grant.Requirement> getDefaultRequirements(GuildDB db, @Nullable DBNation sender,
            @Nullable DBNation receiver, Integer parsed, boolean confirm) {
    ...
    public static List<Grant.Requirement> getRequirements(GuildDB db, DBNation sender, DBNation receiver,
            RawsTemplate template, Integer parsed) {
        List<Grant.Requirement> list = new ArrayList<>();
        list.add(new Grant.Requirement(
                "Days granted must NOT exceed: " + (template == null ? "`{days}`" : template.days), false,
    ...
    public double[] getCost(GuildDB db, DBNation sender, DBNation receiver, Integer parsed) {
        ...
        if (!db.isAllianceId(receiver.getAlliance_id())) {
            throw new IllegalArgumentException("Receiver (" + receiver.getMarkdownUrl() + "/"
                    + receiver.getAllianceUrlMarkup() + ") is not in an alliance registered to this guild. (currently: "
                    + db.getAllianceIds() + ")");
        }
        ...
        for (Transaction2 record : receiver.getTransactions(-1, true, 0L, Long.MAX_VALUE)) {
            if (record.tx_datetime > cutoff && record.sender_id == receiver.getId()
                    && (record.hasNoteTag(DepositType.RAWS) || record.hasNoteTag(DepositType.TAX))) {
        ...
        if (minDate != Long.MAX_VALUE) {
            ...
            double[] revenueOverTime = PW.multiply(revenue,
                    (TimeUtil.getTurn() - TimeUtil.getTimeFromTurn(minDate)) / 12d);
            ...
            for (ResourceType type : ResourceType.values) {
                ...
                if (needed.getOrDefault(type, 0D) > 0 && expected > actual)
                    throw new IllegalArgumentException(
                            "The nation has already received a raw grant within the past " + days + " days");
            }
        }
    ...
    public Integer parse(DBNation receiver, String value) {
        ...
        if (result == null)
            result = Math.toIntExact(days);
        if (result > days) {
            throw new IllegalArgumentException(
                    "Amount cannot be greater than the template days `" + result + ">" + days + "`");
        }
        if (result < 1) {
