...
public enum NationMetric {
    SOLDIER(false) {
        @Override
        public double apply(DBNation alliance) {
    ...
    NationMetric(boolean averageInAggregate) {
    ...
    public static NationMetric[] values = NationMetric.values();
    ...
    public static synchronized void update(int topX) {
        ...
        for (DBAlliance alliance : alliances) {
            for (NationMetric metric : values) {
    ...
    public static TimeNumericTable generateTable(NationMetric metric, long cutoffTurn, Collection<String> coalitionNames, Set<DBAlliance>... coalitions) {
        return generateTable(metric, cutoffTurn, TimeUtil.getTurn(), coalitionNames, coalitions);
    }

    public static TimeNumericTable generateTable(Collection<NationMetric> metrics, long startTurn, long endTurn, String coalitionName, Set<DBAlliance> coalition) {
        Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> metricMap = NationMetric.getMetrics(metrics, startTurn, endTurn, coalition);
        return generateTable(metricMap, metrics, startTurn, endTurn, coalitionName, coalition);
    }

    public static TimeNumericTable generateTable(Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> metricMap, Collection<NationMetric> metrics, long startTurn, long endTurn, String coalitionName, Set<DBAlliance> coalition) {
        double[][] valuesByTurnByMetric = NationMetric.toValueByTurnByMetric(metricMap, metrics, startTurn, endTurn, coalition);

        NationMetric[] metricsArr = metrics.toArray(new NationMetric[0]);
        ...
        {
            int i = 0;
            for (NationMetric metric : metrics) {
    ...
    public static TimeNumericTable generateTable(NationMetric metric, long startTurn, long endTurn, Collection<String> coalitionNames, Set<DBAlliance>... coalitions) {
        ...
        Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> metricMap = NationMetric.getMetrics(Collections.singleton(metric), startTurn, endTurn, coalitions);
        double[][] valuesByTurnByCoalition = NationMetric.toValueByTurnByCoalition(metricMap, metric, startTurn, endTurn, coalitions);
    ...
    public static double[][] toValueByTurnByMetric(Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> metricMap, Collection<NationMetric> metrics, long cutoffTurn, long maxTurn, Set<DBAlliance> coalition) {
        NationMetric[] metricsArr = metrics.toArray(new NationMetric[0]);
        ...
        for (int k = 0; k < metricsArr.length; k++) {
            NationMetric metric = metricsArr[k];
            boolean shouldAverage = shouldAverageArr[k];

            for (DBAlliance alliance : coalition) {
                Map<NationMetric, Map<Long, Double>> metricData = metricMap.get(alliance);
                if (metricData == null) continue;
                Map<Long, Double> membersByTurn = null;
                if (shouldAverage) {
                    membersByTurn = metricData.get(NationMetric.MEMBERS);
    ...
    public static double[][] toValueByTurnByCoalition(Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> metricMap, NationMetric metric, long cutoffTurn, long maxTurn, Set<DBAlliance>... coalitions) {
        ...
        for (Map.Entry<DBAlliance, Map<NationMetric, Map<Long, Double>>> entry1 : metricMap.entrySet()) {
            DBAlliance alliance = entry1.getKey();
            Map<Long, Double> membersByTurn = null;
            if (shouldAverage) {
                membersByTurn = entry1.getValue().get(NationMetric.MEMBERS);
                if (membersByTurn == null) {
                    continue;
                }
            }
    ...
    public static Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> getMetrics(Collection<NationMetric> metrics, long minTurn, Set<DBAlliance>... coalitions) {
        return getMetrics(metrics, minTurn, TimeUtil.getTurn(), coalitions);
    }

    public static Map<DBAlliance, Map<NationMetric, Map<Long, Double>>> getMetrics(Collection<NationMetric> metrics, long minTurn, long maxTurn, Set<DBAlliance>... coalitions) {
        ...
        Set<NationMetric> finalMetrics = new HashSet<>(metrics);
        if (aaIds.size() > 1) {
            for (NationMetric metric : metrics) {
                if (metric.shouldAverage()) {
                    finalMetrics.add(NationMetric.MEMBERS);
                    break;
                }
            }
        }
